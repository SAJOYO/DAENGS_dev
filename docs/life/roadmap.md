...
| D5c | **인용 확장의 `법` 해소 + 항 단위 조회** | RAG-057 ⑥ — 시행령 별표는 근거를 `법 제101조제4항제4호` 로 적는데 맨몸 `법` 이 어떤 `document_title` 과도 안 맞고, 조문을 조 단위로만 읽는데 청크는 항 단위다. **별표 층의 확장 기여가 0이다** | ⬜ DB 무변경이지만 랩 동반 |
...
D5c(인용 확장)는 DB 무변경이라 09-21 전에도 되지만 랩이 붙는다.  D5d 는 사람 확인이 먼저다.
```
