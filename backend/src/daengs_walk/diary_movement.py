...
from .diary_input import DiaryContract, digest
from .diary_route_patterns import RoutePatternBindingPolicy
from .storyboard_input import route_nodes
from .storyboard_selection import session_speed_baseline
...
def prepare_movement(source, route, policy, pattern_policy=None):
    from .diary_route_slots import prepare_route_patterns

    if route is None:
        return None
    # The existing preparation checks source route identity and canonical quality.
    patterns, _ = prepare_route_patterns(
        source, route, pattern_policy or RoutePatternBindingPolicy()
    )
