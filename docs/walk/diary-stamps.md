...
중요한 남은 경계: 현재 `read_input()`은 확정 분석 참조를 읽지만 `observations=[]`다.
이번 선택기는 이미 검증된 관측 후보를 소비한다. 실제 확정 동선에서 체류/속도 재료를 만드는
연결은 생성 런타임 이관 때 필요하다. 후보가 공급되지 않은 입력은 `observation_pool_empty`로
남으며 사용자 기록은 그대로 처리한다.
