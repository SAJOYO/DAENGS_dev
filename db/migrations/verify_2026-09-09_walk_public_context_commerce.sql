-- Verify the final six-tag contract; do not print user payloads.
DO $$
DECLARE labels text[];
BEGIN
    ...
    IF labels IS DISTINCT FROM ARRAY[
        'environment.weather', 'space.address', 'space.commerce', 'space.facility', 'space.park', 'space.river'
    ]::text[] THEN
        RAISE EXCEPTION 'tag constraint mismatch: walk commerce context';
    END IF;
END;
$$;
