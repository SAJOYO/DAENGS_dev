...
_RECEIPT_RULE = """You read a Korean veterinary clinic receipt photo. Only transcribe what is actually printed on the receipt — never invent a value that is not there. Never write the payer's name, a personal phone number, a card number, or a business registration number anywhere in the output, including inside item names — if such a string appears next to an item, drop that part and keep only the item description. Amounts are integers in Korean won; strip commas and the "원" suffix, digits only. hospital_phone is the clinic's own front-desk number, not the payer's — digits and hyphens only. suggested_reason_code must be one of the listed codes or null: when the items alone do not clearly point to one reason, output null (this leaves the confirmation screen with nothing pre-selected, which is the point) — reserve "other" for when the reason is clear but not in the list. Set is_emergency to true only when a line item such as a night-treatment fee, an emergency-treatment fee, or a holiday surcharge is actually printed — never because the treatment merely sounds urgent; this field records printed text, not judgment. When a checkup produced a targeted treatment or test on the same receipt, the body-system code wins over "checkup" — "checkup" is only for a checkup that is not followed by anything. When one visit lists several problems for an older dog, pick the single body system with the largest cost. At the mouth boundary, teeth and gums are "dental" while anything at or below the esophagus is "digestive". At the ear boundary, the outer ear and ear canal are "ear" while the inner ear and vestibular system are "neurologic". Heartworm prevention medication is "parasite_prevention" while treatment of an active heartworm infection is "cardiac". Neuter surgery and pyometra treatment open the same organs and look alike on a receipt — the diagnostic and procedure line items decide which. If the image is not a receipt, or the total cannot be read, output status="unreadable" with the reason instead."""
