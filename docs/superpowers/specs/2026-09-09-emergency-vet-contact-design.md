...
출혈 피를 토 토혈 혈변 혈뇨 각혈
교통사고 차에 치 삼켰 이물질 주워 먹었 주워먹었 주워 먹어
열사병 배가 부풀 소변을 못 난산
초콜릿 양파 포도 자일리톨
...
> "남은 것"에 *"두 글자 일반 명사의 오폭(수술·병원·검사·접종·진단) — D-063 ② 의 결합 규칙이
> 서야 닫힙니다"* 가 있고, D-063 ② 는 *"일반 의료 명사는 context signal 로만 쓰고 단독
> 차단어로 승격하지 않는다"* 다. 위 AMBIG×URG 가 정확히 그 결합 규칙의 한 사례다.
> **D-063 은 2026-09-09 현재 `dev` 에 없다** — #350 브랜치에만 있으므로 이 문서는 그것을
...
- **#350 머지 뒤 재검토.** `fix/training-medical-gate-both-directions` 가 훈련 게이트를
  양방향으로 고치는 중이고(D-063), 서빙 사전을 `load_serving_medical_terms()` 한 곳으로
  합친다. 그 뒤에 물을 것 둘 — ⓐ AMBIG×URG 결합 규칙을 두 게이트가 **같은 구현**으로 쓸지,
  ⓑ 그렇게 합쳐도 §3 의 "whitelist 를 물려받지 않는다"가 유지되는지. **지금은 합치지
  않는다** — #350 이 아직 draft 이고 D-063 이 `dev` 에 없어서, 지금 맞추면 움직이는 것에
  묶인다.
- **골든셋 쏠림 보정** (§5-3).
