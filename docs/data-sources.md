...
| 펫보험 | 3 | **1** (knia 110청크) | 1 (약관은 별도 카드) | knia 비교공시 + 보험사 약관 PDF |
| **문서형 소계** | **23** | **10** | **4** | |
...
2. ⚠️요확인 9건 → **6건.** 운송약관 4 · 접종 2. **보험 3건은 2026-08-28 (#50) 에 다 확인됐다** —
...
4. **robots.txt 차단** — `animal.go.kr`(§2) · `qia.go.kr`(§3) · `e-insmarket`(§7) ·
   **현대해상 공시실**(§7, 2026-08-28 추가 — 펫보험 42상품으로 2위 판매사인데 약관을 못 받는다). 검역본부 두 사이트는
...
- [ ] **`insurer-terms-pdfs`** — 각 보험사 상품공시실 약관 PDF · ✅확인 — **RAG 핵심 코퍼스**
      ...
      → ✅ 시드의 삼성 PDF 는 아직 산다 (200 · 11.3MB · `%PDF-` · 202p)
...
| 회사 | knia 코드 | 상품 | 공시실 딥링크 | 정적 수집 |
| --- | --- | --- | --- | --- |
| 삼성화재 | N08 | 24 | `samsungfire.com/vh/page/VH.REIF0011.do` | ◐ AJAX (PDF 직링크는 확보됨) |
| 메리츠화재 | N01 | 14 | `meritzfire.com/disclosure/product-announcement/product-list.do` | 🚫 SPA(hash 라우팅) |
| 현대해상 | N09 | 42 | `hi.co.kr/bin/CI/ON/CION3200G.jsp` | 🚫 **robots 차단** |
| KB손보 | N10 | 28 | `kbinsure.co.kr/CG802030001.ec` · `/CG803000012.ec` | ◐ AJAX |
| DB손보 | N11 | 53 | `idbins.com/FWMAIV1534.do` | ◐ AJAX |
| 농협손보 | N71 | 44 | `nhfire.co.kr/announce/productAnnounce/retrieveInsuranceProductsAnnounce.nhfire` | ◐ AJAX |
| 카카오페이손보 | N70 | 11 | `kakaopayinscorp.co.kr/products/pet` | 🚫 SPA(805B 껍데기) |

- **현대해상은 robots 로 막힌 쪽**이다 (§12 대로 우회하지 않는다). 42상품으로 두 번째로 큰 판매사인데
  약관을 못 받는다 — knia 의 비교 수치로만 답하게 된다
- 상품 216건은 **판매중 전부**이고 재가입용·다이렉트 변형이 섞여 있다. 대표 상품 1종씩으로 끊는 판단은 유효하다
...
- [ ] `insurer-terms-pdfs` — 보험 약관 PDF (사별 AJAX 역추적 7건이 선행. 별도 카드)
