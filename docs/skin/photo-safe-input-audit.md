...
2. 개발용 앱이 만든 JPEG와 네모를 동의된 테스트 사진에서 확보하여 Android 전처리/좌표를 확인한다.
   기존 STEP36~43의 네모·탐지기 실험을 새로운 기법이라고 반복하지 않는다.
3. 별도 카드에서 개발 환경의 명시적 opt-in 후보 연결을 검토한다. 기존 release·기록 계약,
   인증, 2단계 호출, latency를 포함한다. 원본 수정 후 sync 규칙을 따른다.
4. 개발 검증과 사람 리뷰 이후에만 dev→main 스냅샷 및 운영 반영을 검토한다.
