...
같은 문장을 에이전트 프롬프트(`agent-ko-v3`)에도 넣었습니다 (D-055 ⑦ 규칙 1, 지금은 D-072 로
없어진 규칙입니다). 에이전트 쪽은 이 카드에서 라이브로 재지 않았습니다 — 비교 러너 3회분(240×2 호출)은
이 카드의 예산 밖이고, 두 구현의 결정론 동치는 그 시점의 `tests/test_orchestrator_failure_contract.py`
ⓓ 가 쟀습니다(그 파일도 D-072 로 에이전트와 함께 없어졌습니다).
