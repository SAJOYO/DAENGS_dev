...
def test_prompt_keeps_the_v4_social_rules_without_keyword_lists() -> None:
    ...
    # v7 (PR #204) only added the `place` destination and v8 (PR #279) only added the
    # exclusion sentence; the social rules are intact, which is what the assertions below
    # actually check.
    assert PROMPT_VERSION == "semantic-router-ko-v8"
    prompt = build_semantic_router_prompt(query="고마워", context={})
    assert "PROMPT_VERSION: semantic-router-ko-v8" in prompt
