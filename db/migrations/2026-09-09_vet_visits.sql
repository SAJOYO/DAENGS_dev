...
--
-- 왜: 저장소 탭이 밥·약·간식까지 왔지만(#332) 병원비는 어디에도 안 남는다. 보호자가
-- 영수증을 찍으면 금액·병원·항목을 읽어 두고, **진단 사유는 유저가 확정한 것만** 남긴다.
-- 사유가 닫힌 목록인 이유와 표를 둘로 가른 이유는 db/init/25_vet_visits.sql 머리말에 있다.
--
-- 새 표 둘이라 옛 앱·옛 코드에 영향이 없다 — 코드보다 먼저 적용해도 안전하다.
-- (README 가 권하는 순서도 그쪽이다: 코드가 먼저 나가면 없는 표를 읽어 500 이 난다.)
BEGIN;
...
-- 적용 뒤 verify_2026-09-09_vet_visits.sql 을 같은 DB 에 돌린다.
