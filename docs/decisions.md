...
🔴 **여기 「judge 가 엄한 쪽으로 기웁니다」라고 적었던 것을 정정합니다** (2026-09-07).
사람이 6건을 검토해 **judge 가 전부 맞다**고 했습니다 (일치 6/6). 제가 `t07`·`t19` 를 "표현
트집" 이라고 본 것은 **자료를 열지 않고 눈으로 읽은 판단**이었고, 그것이 정확히 `RAG-075` ②
입니다 — 이 카드가 그 경고를 인용해 놓고 같은 자리에 빠졌습니다. `RAG-075` ③ 과의 수렴
추론도 함께 철회합니다.
...
⚠ **6/6 을 「judge 가 검증됐다」로 읽지 마세요.** 그 6건은 `review` 가 **judge 가 흔들린
자리만 골라 온 것**입니다. judge 가 자신 있게 grounded 로 통과시킨 15건은 아무도 안 봤고,
거기 오탐이 있어도 이 수에는 안 잡힙니다. 표본을 넓힌다면 **그쪽에서** 뽑아야 합니다.
