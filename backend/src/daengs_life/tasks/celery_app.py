...
app.conf.beat_schedule = {
    "recover-territory-photos": {
        "task": "territory.recover_photos",
        "schedule": 30.0,
        "options": {"queue": "territory-vision", "expires": 25},
    },
    "warm-active-grids": {
        "task": "daengs_life.tasks.realtime.warm_active",
        "schedule": 60.0,
        # **1분 뒤엔 다음 틱이 있다.** 그래서 늦게 집힌 메시지는 실행할 가치가 없다 — 데우려던
        # 값은 이미 다음 틱이 데웠거나 발표 주기가 지나 틀렸다. `expires` 가 그 판단을 워커에
        # 맡긴다: 기한이 지난 메시지는 워커가 **실행하지 않고 버린다.**
        #
        # 이것이 실제로 막는 것은 RAG-050 ③ 의 상황이다. Beat 는 서버에서 상시 돌지만 이 큐의
        # 소비자(RT-002 의 워커)는 아직 없어 하루 1,440개가 `celery` 큐에 쌓인다. 그 워커가 뜨는
        # 날 쌓인 수만 개가 몰아서 실행되면 팀 공용 data.go.kr 일 예산(D-019)이 한 번에 나간다.
        # 55초인 이유는 주기(60초)보다 짧아야 "다음 틱 전에 못 집으면 버린다"가 되어서다.
        #
        # ⚠ `queue` 는 넣지 않는다 — 기본 `celery` 큐여야 RT-002 의 워커가 받는다.
        "options": {"expires": 55},
    },
    ...
    #
    # **개정 감지(C3, RAG-054)도 여기 항목을 더하지 않는다.** cadence 가 manual 인 법령은 주기로는
    # 영영 안 받히는데, `crawl_due` 가 고르는 김에 시행일자만 가볍게 조회해(원본은 안 받는다)
    # 바뀐 것만 `trigger='revision'` 으로 받는다. 조회는 "고르기"의 일부라 등록은 여전히 하나다.
    #
