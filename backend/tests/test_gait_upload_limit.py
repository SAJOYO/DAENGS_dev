...
(`uv sync`, `--group gait` 없음)로 화면과 계약만 보는 개발자도 이 경로를 확인할 수
...
def client(monkeypatch):
    ...
    from daengs_gait import service as serve  # noqa: E402
...
def test_rejects_oversized_without_model_extra(client, monkeypatch):
    ...
    `--group gait` 을 설치하지 않은 상태를 흉내 냅니다. 크기 검사가 import 뒤로
    밀리면 이 테스트가 413 대신 ImportError 를 받아 깨집니다.
    """
    monkeypatch.setitem(sys.modules, "daengs_gait.pipeline", None)
