...
- **`clarify.missing` 은 되묻기의 *종류*, `clarify.missing_axes` 는 *무엇을 물었는지* 입니다.**
  좌표 게이트는 `missing=["location.lat"]` 을, 관찰 되묻기는 `missing=["observation"]` 을
  넣습니다 — 두 어휘를 한 목록에 섞지 않으려는 분리입니다. `missing_axes` 는 `ObservationAxis`
  (APPETITE · ENERGY · STOOL · VOMIT · BREATHING · MOBILITY · OTHER) **1~2개**이고,
  `#416` 의 Turn Resolver 가 후속 답변을 앞 질문에 묶을 때 한국어를 다시 파싱하지 않으려고
  있습니다.
  ⚠ **이 값은 "아직 물어본 항목" 이지 반려견의 상태가 아닙니다.** `APPETITE` 가 있다는 것은
  "식욕을 물었다" 는 뜻이지 "식욕에 문제가 있다" 가 아니며, 어디에서도 반려견의 사실이나
  기록으로 저장되지 않습니다.
...
| General `kind=refuse` — 진단 · 약 이름/용량/시작 여부/부작용 · 응급 · 제도/수치 · 도메인 밖 | **REFUSED** — `refusal.code` 는 사유 범주, `refusal.message` 는 코드가 쓴 고정 안내("수의사에게" / "제도 정보 기능에") |
...
14. **능력 이름의 사본 세 벌은 항상 같은 값을 가집니다** (#269). 능력을 추가할 때 다음
    ...
    라우터가 **고를 수 없는** 능력입니다. `skin` 은 그래도 세 길로 들어옵니다: 명시 신호,
    라우터가 낸 `skin` HANDOFF 를 planner 가 해설로 바꾸는 길(D-081), 그리고 판정 기록이
    붙은 이어 묻기의 `general` 하나짜리 계획을 바꾸는 길(D-083)입니다. 셋 다 planner 가
    정하며, 라우터의 출력 스키마는 어느 쪽으로도 넓어지지 않습니다.
