...
없어서, `dev` 에 머지돼도 서버에서 아무것도 안 돕니다. 의도한 것입니다 (D-016).
...
"판단할 수 없습니다 → 진료 권함" 으로 끝냅니다. 자세한 근거는 `docs/decisions.md` D-017.
...
* 복사 시점 원본 커밋: **`4a8c200`** (2026-08-26)
* ⚠️ **다만 `serve.py` 두 곳은 사본이 앞서 있습니다.** 여기로 옮기면서 발견해
  고친 것이고, 원본에는 아직 안 들어갔습니다 — 원본에도 반영해야 합니다.
  1. `from src import env` 를 추가. 없으면 stdout 이 파이프·파일·컨테이너
     로그로 갈 때 코드페이지가 cp949 가 되어, 시작 로그의 `⚠️` 하나 때문에
     서버가 `UnicodeEncodeError` 로 **뜨지도 못합니다.** 터미널에서 직접 돌릴
     때만 멀쩡해서 안 보이던 버그입니다
  2. `--mock` 이 임시 파일 대신 PIL 이미지를 그대로 넘기게. Windows 는 열려
     있는 `NamedTemporaryFile` 을 다시 못 열어서, **모든 요청이 "판단이 어려운
     사진입니다" 로 나왔습니다** — 팀이 쓸 바로 그 모드가 깨져 있었습니다
