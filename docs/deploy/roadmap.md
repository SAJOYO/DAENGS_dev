...
쓰지 않기로 하면서 아래 1번이 **지금 열린 카드**가 됐다 — Life 로드맵 §4 의 13번이 그것을 가리킨다.
2~4번의 순서는 안 바꿨다.

1. **크롤러·코퍼스·적재 이전** — 🟢 **열린 카드다** (Life §4 의 13번). `DAENGS_CORPUS_DIR` 정본 컷오버(+로컬 워커 정지 **같은 날**),
   crawler-worker·beat 기동, 증분 적재는 VM 에서 CPU 로(`rag load`), 전체 재적재만
   스팟 GPU 또는 개발 PC. 이걸로 "코퍼스 갱신이 개발 PC 에 묶이는" 구조적 약점 해소.
   **그때까지의 임시 절차**는 `runbook.md` §6 의 "Life 코퍼스만 동기화 (GCP)" 다
   (`#290` 이 적었다 — 🔴 통째 덤프·복원이 아니라 `documents` 테이블만 갈아 끼운다.
   그 길은 운영 데이터를 지운다). **이 항목이 없애는 것이 바로 그 손작업이다**
