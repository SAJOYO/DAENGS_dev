...
## 9. 후속 결정 — 일반 질문은 `gemini-3.1-flash-lite` 가 처리 (2026-09-04)

8절 1번을 채택합니다. `NO_ELIGIBLE_SOURCE` 를 받아들이고 목욕 축은 Training RAG 범위 밖에 남깁니다.
대신 **일반 질문은 `gemini-3.1-flash-lite` 의 내부 지식으로 답하는 별도 capability** 가 처리합니다.
RAG 코퍼스도, 웹 검색 API 도 쓰지 않습니다.

동기는 목욕이 아닙니다. 테스터 피드백에서 "산책 몇 번", "어떻게 하면 건강해질 수 있어요?" 류가
전부 `aggregate.py` 의 빈 RoutePlan → `실행하거나 안내할 수 있는 기능이 없습니다.` FAILED 경로로
떨어져 봇이 쓸 수 없다는 문제가 먼저 있었고, 그 해소책으로 라우팅 문서 "옵션 C"(care capability 없음)를
뒤집은 것입니다. 목욕 질의는 그 분류의 결과로 `general` 에 떨어질 뿐, 별도 특례가 아닙니다.

**6절과 충돌하지 않습니다.** 6절이 기각한 "새 Care capability 신설"은 목욕 검색을 성공처럼
보이게 만드는 우회로로서의 기각이었습니다. 이 capability 는 6절의 금지 항목을 하나도 도입하지
않습니다 — `목욕` → `document_id` 하드코딩 없음, 강제 문서 주입 없음, 유사도 임계·전역 top-k
변경 없음 (`ChatRequest.top_k` 는 `le=4` 유지), 서빙 매니페스트 추가 없음, 직접 근거·거절 규칙
완화 없음. Training RAG 의 근거 계약은 그대로입니다.

**남는 안전 과제 하나.** 목욕 혐오는 2절 기준 ⑤(강압·플러딩 회피) ⑥(스트레스 신호 인지와 중단)
⑦(물기 시도 시 안전한 대응)을 포함하고, `목욕할 때 물려고 해요` 는 교상 위험 질의입니다.
일반 capability 의 기존 경계(진단·용량 금지, 응급 → 수의)는 "행동 조언이 교상을 악화시키는" 축을
덮지 않습니다. 해당 capability 카드에서 행동 악화 경계를 명시하고, 답변을 인용 근거가 아닌
**일반 안내**로 표기해야 합니다.
