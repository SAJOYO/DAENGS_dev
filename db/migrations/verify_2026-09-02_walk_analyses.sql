...
-- 계산된 산책과 아직인 산책.
SELECT analysis_state, count(*) AS walks
FROM walks
GROUP BY analysis_state
ORDER BY walks DESC;

-- 같은 산책을 여러 규칙으로 계산해 둔 것. 1보다 크면 규칙이 바뀐 적이 있다는 뜻이고,
-- **그것이 이 표가 UNIQUE 를 여섯 칸으로 잡은 이유**다.
SELECT count(*) AS analyses,
       count(DISTINCT walk_id) AS walks_analysed,
       max(versions) AS max_versions_per_walk
FROM walk_analyses,
     LATERAL (SELECT count(*) AS versions FROM walk_analyses a2
              WHERE a2.walk_id = walk_analyses.walk_id) AS v;
