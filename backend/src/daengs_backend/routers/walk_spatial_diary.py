...
@router.post("/sheets/query", response_model=WalkRecordSheetsResponse)
async def query_walk_record_sheets(
    body: WalkRecordSheetsRequest,
    user: CurrentAppUser,
    session: Annotated[AsyncSession, Depends(get_snapshot_session)],
) -> WalkRecordSheetsResponse:
    """현재 계정의 명시적 산책별 봉인 원판을 원본 그대로 읽습니다."""
    try:
        items = await diary_service.query_record_sheets(
            session, user.app_user_id, body.client_session_ids
        )
        ...
        logger.exception("walk record sheet incomplete (app_user=%s)", user.app_user_id)
        raise HTTPException(
    ...
    return WalkRecordSheetsResponse(
        items=tuple(WalkRecordSheetResponse(**vars(item)) for item in items)
    )
