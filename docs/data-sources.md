...
| 동반 이동 | 4 | **1** (코레일 2건 · RAG-036) | 3 (`srt`·`seoulmetro`·항공사 — #51) | 코레일 운송약관 + SRT·서울교통공사 |
...
3. ~~PDF 파서 미정~~ — **해소하고 다 썼다.** `PyMuPDF` 로 확정(RAG-034) → 포맷 층 구현(RAG-036, #49)
   → **코레일 2건 · 삼성 약관 11건 수집 완료**(RAG-039, #53). `pdf-entry` 대기 항목은 남지 않았다.
   ⚠️ 다만 **보험약관은 한 파일에 관계법령 전문이 붙어 있어** 앞뒤를 잘라야 한다 (RAG-039 ②) —
...
- [ ] **`insurer-terms-pdfs`** — 각 보험사 상품공시실 약관 PDF · ✅확인 — **RAG 핵심 코퍼스**
      ...
      → ✅ **삼성 수집 완료** (2026-08-29, #53 · RAG-039) — 판매중 11건 · 25.1MB · 1,530쪽 ·
      ...
      → ⚠️ **약관 PDF 한 파일에 관계법령 전문이 붙어 있다.** 그대로 읽으면 신용정보법 조문이
        "펫보험 약관 제32조" 로 인용된다. 앞의 안내 책자와 뒤의 별표·관계법령을 잘라낸다 (RAG-039 ②)
...
| --- | --- | --- | --- | --- | --- |
| 삼성화재 | N08 | 6 | 24 | `samsungfire.com/vh/page/VH.REIF0011.do` | ✅ **수집 완료** (RAG-039) |
...
- [x] `insurer-terms-pdfs` — 보험 약관 PDF **삼성 11건 완료** (RAG-039). 남은 6사는 별도 카드
