...
from daengs_walk.diary.board.models import ObservationCore, RecordCore
...
from daengs_walk.diary.contracts.slots import SlotDecision, SlotEvidence, SlotSource
    ...
    for saved in sorted((*source.backgrounds, *extra_backgrounds), key=lambda b: b.id):
        ...
        if part == "space":
            if saved.payload_schema == "space-materials-v1":
                from daengs_walk.diary.slots.space import space_candidates
            ...
            if saved.payload and saved.payload.get("format") == "kma-grid-temperature-v1":
                from daengs_walk.diary.slots.temperature import temperature_candidate
