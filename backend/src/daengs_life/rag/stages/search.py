...
class Hit:
    ...
    # 인용 확장으로 딸려 온 청크면 **그것을 끌어온 청크의 `chunk_id`** (RAG-036).
...
def expand_citations(hits: list[Hit], query: Query, *, include_supplementary: bool = True,
                     limit: int = MAX_EXPANDED, conn=None) -> list[Hit]:
...
def search(query: Query, *, k: int = DEFAULT_K, include_supplementary: bool = True,
    ...
    **`include_supplementary` 기본이 `True` 인 것은 결정이다** (RAG-026 ①). 부칙은 세 모델 모두에서
    ...
    try:
        ...
        # **인용 확장을 인자로 빼지 않는다** (RAG-036). 빼면 CLI·9단계·FastAPI 가 각자 켜고
        # 끄게 되고, 그 순간 검문소③이 본 것과 서빙이 하는 것이 갈린다 — 이 파일이 처음부터
        # 막고 있는 그 자리다. `test_search` 가 단언하는 시그니처도 그대로 남는다.
        return expand_citations(hits, query, include_supplementary=include_supplementary,
                                conn=conn)
    finally:
        if own:
            conn.close()
