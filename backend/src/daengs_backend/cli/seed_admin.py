...
**최초 1회용입니다** (A3 · #207 이후). 두 번째 계정부터는 콘솔의 `관리자 계정` 화면
(`POST /admin/admins`)에서 발급하세요 — 그쪽은 감사 기록에 남고 여기는 안 남습니다.
이 명령이 남아 있는 이유는 **아직 아무 계정도 없는 DB** 때문입니다. 계정을 만들려면
ADMIN 으로 로그인해야 하는데, 첫 계정은 그 로그인을 할 수 없습니다.
...
def main() -> None:
    ...
    print("--   서버 PC:  docker compose exec -it pgvector psql -U daengs -d vectordb")
    print("--   개발 PC:  psql -h <POSTGRES_IP> -U daengs -d vectordb")
    print("--")
    print("-- admin_users 는 daengs 소유라 daengs 로 넣으면 됩니다.")
    print("-- (postgres 소유인 documents 와 다릅니다 - 그쪽은 볼륨 초기화 때 만들어졌습니다)")
    print(f"-- {note[3:]}")
    print("-- ─────────────────────────────────────────────────────────────")
    print(statement)
    print()
