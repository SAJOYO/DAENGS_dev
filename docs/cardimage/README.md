...
## 지금 상태 (2026-09-16, #572 12달 열기 + 2장 뽑기 — 구현·리뷰 완료, 머지 대기)

**다음 카드와 진행도는 [`roadmap.md`](roadmap.md) 맨 위 표를 본다.**

**#572 (09-16, D-084).** #557 실험이 찾은 것을 제품에 박고 열린 달을 4·9월 둘에서 **1~12월 전부**로
넓혔다(`DAENGS_CARDIMAGE_MONTHS` 기본값 `1..12`). 달마다 `catalog.MonthCard.seeds` 에 육안으로
검증된 seed 목록을 두고 그중에서 뽑는다. 한 요청에 카드 **2장**(순차, `pick_group` 으로 묶임)을
만들어 `POST /app/ai-cards/{id}/choose` 로 하나만 고르고 나머지는 지운다. 한도(D-077)를
...
(`compare-2026-09-16-months-seeds.md`). 상세는 `worklog.md` 09-16 절, seed 목록·발견은 같은 문서
「GPU 실험」절.
...
| 카드 종류별 엔진 | **운영은 지금도 Nano Banana 2 한 장뿐이다.** #572(12달 열기 + 2장 뽑기)는 seed 목록·2장 고르기 같은 FLUX.2-klein-4B 쪽 설계를 코드로 옮겼지만 `DAENGS_CARDGEN_URL` 은 **이 카드에서도 켜지 않는다** — 켜기 전에 4월 강아지 정체성 결함(아래 「지금 상태」)부터 풀어야 한다 | #572 D-084, `compare-2026-09-16-months-seeds.md` |
...
| 한도 규칙 | 동시 1장 · 강아지마다 달마다 한 장(보호자마다 따로, 지우면 그 달은 다시 열림) · 제목에만 쓰는 `title_name` · 목록의 남은 횟수 — **D-077, #572 로 D-084 가 개정.** 세는 단위가 카드 장수에서 **요청(`pick_group`) 하나**로 바뀌었다 — 요청은 카드 최대 `cardimage_pick_count`(2)장을 순차로 만들고 하나를 고른다. 닮음 미달 요청은 하루 한도를 안 쓰되 지울 수 없는 시도 표시로 유료 실패 상한(5)에는 걸린다 | 사용자 결정 09-15 (#543) · D-084(#572, 09-16). 앱 짝은 `DAENGS_APP#414` |
