...
## 2. 지금 상태 (2026-09-05, `origin/dev` `5d9d469` · **운영 DB 직접 실측**)
...
> 감사 로그(#221) · 비밀번호(#222) · 운영 지표(#223) · **신고(#235 · #237)**. `main` 은 `dev` 보다 108 커밋 뒤다.
...
행 수 (내용은 열지 않고 `count(*)` 만): `app_users` 23 · `chat_sessions` 16 · `chat_turns` 37 ·
`chat_summaries` 7 · `admin_audit_log` 4.

**두 가지가 여기서 갈린다.** ⓐ A1 이 기다리던 게이트(`chats` · `admin_audit_log`)는 **이미 열려
있었다** — 로드맵이 09-03 뒤로 실측되지 않아 계속 "적용 대기" 로 적혀 있었을 뿐이다.
ⓑ 그렇다고 **운영 DB 가 `dev` 와 같은 것은 아니다** — 위 표의 미적용 둘이 있고, 규칙은
