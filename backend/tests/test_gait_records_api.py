...
def test_list_requires_dog_id(gait):
    ...
    assert gait.client.get("/v1/records").status_code == 400
...
def test_list_filters_by_dog_and_sorts_oldest_first(gait):
    ...
    body = gait.client.get("/v1/records", params={"dog_id": "dog-1"}).json()
...
def test_list_omits_heavy_and_forbidden_fields(gait):
    ...
    gait.make(ID_A, "dog-1")
    row = gait.client.get("/v1/records", params={"dog_id": "dog-1"}).json()["records"][0]
...
def test_list_paginates_with_cursor(gait):
    ...
    first = gait.client.get("/v1/records", params={"dog_id": "dog-1", "limit": 2}).json()
...
def test_stale_cursor_is_rejected(gait):
    ...
    gait.make(ID_A, "dog-1")
    r = gait.client.get("/v1/records", params={"dog_id": "dog-1", "cursor": ID_C})
...
def test_limit_bounds(gait, limit):
    gait.make(ID_A, "dog-1")
    r = gait.client.get("/v1/records", params={"dog_id": "dog-1", "limit": limit})
    assert r.status_code == 400


def test_empty_list_is_not_an_error(gait):
    body = gait.client.get("/v1/records", params={"dog_id": "nobody"}).json()
...
def test_delete_removes_record_and_videos(gait):
    ...
    body = gait.client.delete(f"/v1/records/{ID_A}").json()

    assert body["deleted"] == {"record": True, "original": True, "overlay": True}
    assert not original.exists()
    assert not overlay.exists()
    assert gait.client.get(f"/v1/records/{ID_A}").status_code == 404


def test_delete_missing_record_is_404(gait):
    assert gait.client.delete(f"/v1/records/{ID_C}").status_code == 404
...
def test_delete_reports_partial_failure_instead_of_silent_success(gait, monkeypatch):
    ...
    r = gait.client.delete(f"/v1/records/{ID_A}")
...
def test_existing_endpoints_still_work(gait):
    ...
    assert gait.client.get(f"/v1/records/{ID_A}").status_code == 200
    assert gait.client.get(f"/v1/records/{ID_C}").status_code == 404
    assert gait.client.get(f"/v1/records/{ID_C}/overlay").status_code == 404

    # compare 는 없는 기록을 404 로 막는 것까지가 이 파일의 범위입니다
    # (계산 자체는 test_gait_inference.py 가 봅니다 — 거기는 모델 그룹이 필요합니다).
    r = gait.client.post("/v1/compare", json={"record_id_a": ID_A, "record_id_b": ID_C})
