...
--
-- 한 칸짜리 마이그레이션이다(#572 Task 5, D-084). below_judge_min 은 boolean · NOT NULL ·
-- 기본 false 여야 한다. 기본값이 없거나 true 면 사용 기록을 적는 쪽이 이 칸을 빠뜨리는 순간
-- (옛 코드 · 2026-09-15 백필 재실행) 그 줄이 미달 표시가 되거나 INSERT 가 죽는다 — 하루 한도가
-- 아무것도 안 세게 되거나 카드 완료 기록이 실패한다.
DO $verify$
DECLARE
    relation regclass;
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE a.attrelid = relation AND a.attname = 'below_judge_min'
          AND a.attnum > 0 AND NOT a.attisdropped
          AND format_type(a.atttypid, a.atttypmod) = 'boolean'
          AND a.attnotnull
    ) THEN
        RAISE EXCEPTION 'column mismatch: ai_card_usage.below_judge_min (want boolean, not null)';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'column mismatch: ai_card_usage.below_judge_min default must be false';
    END IF;
END
$verify$;
...
SELECT below_judge_min, count(*) AS rows FROM ai_card_usage GROUP BY below_judge_min ORDER BY below_judge_min;
