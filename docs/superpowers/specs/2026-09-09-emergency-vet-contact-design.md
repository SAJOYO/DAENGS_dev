...
> **열려 있는 #350 과 같은 모양이다.** `fix/training-medical-gate-both-directions` 의
> "남은 것"에 *"두 글자 일반 명사의 오폭(수술·병원·검사·접종·진단) — D-064 ② 의 결합 규칙이
> 서야 닫힙니다"* 가 있고, D-064 ② 는 *"일반 의료 명사는 context signal 로만 쓰고 단독
> 차단어로 승격하지 않는다"* 다. 위 AMBIG×URG 가 정확히 그 결합 규칙의 한 사례다.
> **D-064 은 2026-09-09 현재 `dev` 에 없다** — #350 브랜치에만 있으므로 이 문서는 그것을
> 확정된 결정으로 인용하지 않는다. #350 이 머지되면 두 규칙의 어휘와 모양을 맞춰야 하고,
> 그때 이 게이트가 `emergency.py` 를 계속 따로 소유할지 다시 본다 (§6).
> (#350 본문이 한때 `D-063` 이라 적고 있었으나 2026-09-09 에 `D-064` 로 정정됐다.
> 그 브랜치의 `decisions.md` 에 실제로 들어간 것도 `## D-064` 다.)

### 3-1-1. #350 의 의료 어휘와 겹치는 자리

응급 게이트가 라우터 앞에 서고 **배타 실행**이라, 여기서 켜진 낱말은 훈련·의료 게이트에
도달하지 못한다. 그래서 #350 이 실을 `medical_terms_v1_curated.json`(50개)과의 부분일치
충돌을 확인했고, 걸린 것이 둘이다.
...
`evals/orchestration_emergency/gold_v1.jsonl` 을 만든다. 씨앗은 위 21건이지만 **그대로 두면
안 된다.**
...
- **#350 머지 뒤 재검토.** `fix/training-medical-gate-both-directions` 가 훈련 게이트를
  양방향으로 고치는 중이고(D-064), 서빙 사전을 `load_serving_medical_terms()` 한 곳으로
  합친다. 그 뒤에 물을 것 둘 — ⓐ AMBIG×URG 결합 규칙을 두 게이트가 **같은 구현**으로 쓸지,
  ⓑ 그렇게 합쳐도 §3 의 "whitelist 를 물려받지 않는다"가 유지되는지. **지금은 합치지
  않는다** — #350 이 아직 draft 이고 D-064 이 `dev` 에 없어서, 지금 맞추면 움직이는 것에
  묶인다.
- **골든셋 쏠림 보정** (§5-3).
