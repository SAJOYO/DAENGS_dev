...
- **이 폴더는 사본입니다.** 원본은
  ...
  이고 복사 시점 커밋은 `3de4640` (main, 2026-09-06) 입니다.
...
- **두 단계가 서로 다른 백본을 쓸 수 있습니다** — 지금은 1단계 `effnetv2_s` /
  2단계 `convnextv2_base` 입니다 (STEP 16 확정). 백본은 하드코딩하지 말고 **폴더 이름에서**
...
- **이 폴더는 배포에 붙어 있습니다** (D-039 · D-040). nginx 가 `/screen/` 을 접두사째
  backend 로 넘기고, 앱이 `POST /screen/v1/screen` 을 부릅니다. 콘솔에서도
  `기능 / 검색 점검 > 피부 스크리닝` 으로 두들겨 볼 수 있습니다 (`/api/screen/*`).
  ⚠️ 가중치(`SCREENING_RELEASE_DIR`)가 없으면 **503** 입니다 — 라우터가 죽은 게 아닙니다.
