...
**15건이 네가 볼 것이다.** 이 15건은 judge 가 전부 `grounded=true` 로 통과시킨 자리다.
그 사실을 알고 시작하되 **그것에 끌려가지 마라** — 오탐이 숨는다면 정확히 여기다.
...
- **네가 false 를 준 문항** — 여기가 이 작업의 값이다. judge 가 통과시킨 것을 네가 걸었다면
  둘 중 하나다: judge 의 오탐을 네가 잡았거나, 네가 너무 엄하거나. `chunks` 를 직접 인용해서
  어느 쪽인지 보여라. **인용 없이 "없어 보인다" 고 쓰지 마라** — 그 판단이 이 카드에서 이미
  네 번 틀렸다 (D-060 ⑥ · `RAG-075` ②).
