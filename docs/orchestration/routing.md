...
HANDOFF, D-074). 셋 다 라우터를 아예 안 거치고 계획을 냅니다. 마지막 것이 **이 저장소에서
...
### Place 를 "그 장소가 개를 위한 것인가" 로 좁힘 — `semantic-router-ko-v10` (CONFIRMED — D-061, 2026-09-07)

#277 의 답변 품질 측정에서 **도메인 밖 21건 중 6건이 답으로 나갔습니다.** D-058 ④ 가 "문구가
아니라 라우터 정책의 몫" 이라며 넘긴 자리입니다. 대조군을 낀 12건 프로브(유료 24건, 전/후)로
열어 보니 **새는 모양이 하나뿐이었습니다 — 전부 `place`.**

| 질의 | v9 | v10 |
| --- | --- | --- |
| `근처에 고양이 키우는 카페 있을까?` | **Place (답함)** | 빈 선택 → 안내 |
| `근처에 고양이 사료 파는 곳?` | **Place + General** | 빈 선택 → 안내 |
| `배가 아픈데 근처 내과 찾아줘` | **Place (답함)** | 빈 선택 → 안내 |
| `근처 이탈리안 레스토랑 추천해줘` | **Place (답함)** | 빈 선택 → 안내 |
| `주식 어떻게 대비하지? 그리고 강아지 동반 카페도` | Place | Place (유지) |
| `저녁 메뉴 추천 + 지금 산책 나가도 되는 날씨?` | Walk | Walk (유지) |
| `고양이도 키우는데 우리 강아지 급여는?` | General | General (유지) |
| `고양이는 산책 안해도 되나?` | General → 거절 | General → 거절 |

**안전 프롬프트는 안 건드렸습니다.** `general.py` 에 닿은 것은 이미 제대로 거절하고 있었고
(위 마지막 줄), 문제는 `place` 가 그 앞에서 가로챈 것이었습니다. v9 의 Place 정의는 "장소를
찾아 달라는 말" 을 읽고 **"그 장소가 무엇을 위한 것인가" 를 안 읽었습니다.** v10 은 세 문장을
더합니다 — ⓐ 장소는 개를 위한 것이어야 한다(동반 가능 장소 · 개 용품 · 동물병원 · 미용 · 훈련),
ⓑ 다른 동물(고양이 등)이 주체인 장소·돌봄 질문은 "개 얘기가 아닌 것" 에 든다, ⓒ 근처에서
찾아 달라는 말 자체는 Place 의 근거가 아니다.

**과교정이 이 변경의 진짜 위험입니다.** "주식 얘기 + 강아지 동반 카페" 가 Place 를 잃으면
손해라, 다중 의도 보존 문장을 같은 문단에서 다시 말하고 프로브의 대조군으로 박아 뒀습니다
(`test_orchestration_care_boundary.py`).

**동결 골드 80건 회귀 (runner_v11 · 유료 80건):** raw PASS / general-stripped PASS.
`exact_route_plan_match` 는 v10 과 **똑같은 0.9875** 이고, 틀린 한 건이 `walk_03` → `mixed_09`
로 옮겨 갔습니다. `walk_03`("이 위치의 … 걷기 좋은 …")이 붙이던 군더더기 `place` 가 사라진 것은
이 카드가 노린 방향이고, `mixed_09` 는 v4·v5 run 부터 흔들려 온 경계 사례입니다.
`exact_mixed_execute_handoff_match` 가 1.0 → 0.90 인 것은 그 한 건이 mixed 부분집합으로 옮겨
갔기 때문이며 게이트(≥0.90)는 통과합니다 — **튜닝하지 않고 적습니다** (D-057 ③).

**남은 것.** 정책은 "개 얘기가 아니면 아무것도 고르지 않는다" 인데, 실제로는 라우터가
`general` 을 골라 그 어댑터가 `off_topic` 으로 거절합니다. **사용자가 보는 문장은 같지만
경로가 다릅니다**(FAILED 대신 REFUSED) — 도메인 밖 요청 하나에 Gemini 호출이 한 번 더 붙습니다.
값이 문제가 되면 그때 좁히면 되고, 지금은 관측에서 구분됩니다.
...
- HANDOFF 는 실패가 아닙니다. multipart 이미지·영상 워크플로(Skin·Gait)는 전용 API 에
  ...
  **세 번째 target 이 있습니다 — `care_log`** (D-074, `reason: "care_log_entry_required"`).
