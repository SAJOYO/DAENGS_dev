...
| 글자 | **이름·제목은 AI 가 아니라 Pillow 가 그린다** (`backend/tools/cardimage_title.py`). 제목 형식 `BLOSSOM <이름>`. 왼쪽 정렬 x=268, 대문자 띠는 배지 `26APR` 과 세로 중심 일치(106.5), 검은 판의 기울어진 오른쪽 경계 안에서 길면 축소. 은색 그라데이션+외곽선+그림자 | 사용자 결정 09-14. 한글 깨짐·글꼴 불일치·길이 문제 회피. 원본 픽셀을 재서 맞춤 |
