-- ---------------------------------------------------------------------
-- dog_cards : 뽑아 놓은 도감 카드 (D-052)
-- ---------------------------------------------------------------------
-- 앱이 Room(`drawn_card`)과 `filesDir/cards/<id>.png` 에만 갖고 있던 것을 서버로
-- 올린다. 그전까지는 **폰을 바꾸면 뽑은 카드가 전부 사라졌다.**
--
-- ⚠️ **id 를 서버가 만들지 않는다.** 앱이 만든 UUID 를 그대로 받는다 — 앱의
--    `DrawnCardRow.id` 주석이 "서버가 붙어도 이 id 를 그대로 올려서 **재전송이
--    멱등해진다**" 라고 적어 둔 그대로다. 그래서 이 표의 쓰기는 POST 가 아니라
--    **PUT upsert** 이고, 같은 카드를 두 번 올려도 한 장이다.
--    (다른 도메인은 backend 가 키를 만든다 — 원칙 6. 여기만 예외인 이유는 카드가
--     **오프라인에서 먼저 만들어지기** 때문이다. 로그인 없이 둘러보기로도 뽑는다.)
--
-- ⚠️ **결과를 저장하고 시드를 저장하지 않는다.** 카드는 "그때 그 사진 + 그때 뽑힌
--    야채"라 다시 만들 수가 없다. 확률표를 고쳤다고 이미 가진 카드가 바뀌면 안 된다
--    (앱 주석과 같은 이유).
--
-- ⚠️ **원본 사진은 저장하지 않는다.** 저장소에 올라가는 것은 누끼 딴 얼굴 PNG 하나뿐이다 —
--    배경이 통째로 날아가서 뒤에 찍힌 집도 사람도 같이 지워진다. 원본보다 안전하다.
