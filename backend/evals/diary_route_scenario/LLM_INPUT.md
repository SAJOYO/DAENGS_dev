...
2026-09-14, #508. `diary-prose-input-v1`을 현재 카드 작성 오케스트레이션과
마지막 제목 갱신 실험에 연결했다. 공공자료 수집·슬롯 선정 결과는 그대로 두고
LLM이 읽는 표현을 별도로 만든다.
...
선정된 슬롯 / 확정 본문
  ...
  → walk_diary_llm.normalize (허용 필드만 새 객체로 생성)
...
- 행동: 확인된 행위자 이름(미확인이면 null)과 기록된 행동만 보낸다.
- 현재 카드 제목: 확정된 공간/행동 문장, 주소 참조, 관측 설명만 보낸다.
  관측의 대상과 의미는 유지하고 원 관측 ID·출처·내용 버전은 서버에 둔다.
- 마지막 장면 제목 실험: 전체 본문과 순서·시각·출발/종료 역할을 함께 보낸다.
  사용자 메모를 포함하는 기존 실험 정책은 유지한다.
- 공간 재료는 `m1`, 제목 대상은 `c1`부터 시작한다. 서버가 호출마다 별도 참조표를
  소유한다. 모델에 원래 장면/행동 ID나 64자리 버전을 복사하도록 요구하지 않는다.

`walk_diary_llm_materials.py`는 필드 허용 목록을 사용한다. 원자료에 새로운 필드가
...
정규화는 재료를 재선정하거나 새로운 사실을 추론하지 않는다. 기존 실험의 관측·동선
재료가 공간 작성기로 들어가지 않는 경계도 유지한다. 속도 기준/동선 통합은 이 변경의
대상이 아니다. 다른 레거시 일기 작성기는 이 어댑터의 적용 대상이 아니다.
