...
후속 [후보 조회 API](trajectory-api.md)(#475)가 같은 검증 입력과 어댑터를 재사용한다.
아래는 그림자 계산 자체의 구현·검증 범위이며, 새 API도 활성 결과를 변경하지 않는다.
...
해제한 뒤 CPU 계산으로 넘어간다. 이 단계에서는 HTTP endpoint나 자동 백필을 추가하지 않았고,
후속 HTTP 경계는 위 후보 조회 문서에 별도로 명시했다.
