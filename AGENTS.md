...
uv run pytest              # 전체 기본 스위트 — 아래 검증 기준에 따라 필요할 때 실행
...
**변경의 영향을 확인하는 가장 작은 테스트 묶음부터 실행합니다.** 매 수정마다 전체
스위트를 돌리지 않습니다. 파일 이름만 맞추지 말고 변경된 함수의 호출자, 공통 fixture,
의존성 주입, 요청·응답 계약을 확인해 대상을 고릅니다. 실패를 수정할 때는 해당 실패를
재현하는 케이스부터 시작하고, 수정 뒤 관련 회귀 테스트를 실행합니다.

### 선택과 확대 기준

| 변경 범위 | 작업 중 확인할 대상 |
| --- | --- |
| 한 기능의 내부 로직 | 해당 테스트 파일 또는 관련 케이스. 동작 변경을 검증하는 회귀 테스트 |
| 서비스·라우터·요청/응답 계약 | 해당 기능 + 이를 호출하는 API·계약·직렬화 테스트 |
| 공통 인증·DB 접근·설정·공통 fixture·앱 조립 | 직접 사용하는 기능/패키지까지 확대. 영향 범위를 한정하기 어렵거나 여러 패키지에 걸치면 전체 기본 스위트 |
| 의존성·lock·패키지 구성·pytest 설정 | 영향받는 패키지의 수집·실행. 공통 런타임이나 수집에 영향을 주면 전체 기본 스위트 |
| SQL·마이그레이션·공간 쿼리 | 관련 DB 통합 테스트와 마이그레이션 검증. 순수 단위 테스트 통과만으로 SQL 검증을 대신하지 않음 |
| 문서만 변경 | diff·상대 링크·기재한 명령/경로 확인. 실행 코드 변경이 없는 문서 검증에 전체 앱을 띄우지 않음 |

테스트 대상이 없거나 기존 테스트가 변경 동작을 다루지 못한다면, 의미 있는 동작 테스트를
추가합니다. 구현의 필드나 코드 모양만 그대로 반복하는 테스트는 만들지 않습니다. 문서나
단순한 표시 문구 같은 되돌리기 쉬운 변경에 형식적인 테스트를 추가하지 않습니다.

### 실행 예시

모든 Python 명령은 `backend/`에서 `uv run`으로 실행합니다. 아래 경로는 선택 방식의
예시이며 모든 변경에 이 묶음을 기계적으로 적용하지 않습니다.

```powershell
# 한 기능부터 시작
uv run pytest -q tests/test_territory_attempts.py

# 방문 인증 서비스와 사진 판정 연결을 바꿨다면 관련 경계까지
uv run pytest -q tests/test_territory_attempts.py tests/test_territory_vision.py

# 영향받는 패키지로 확대
uv run pytest -q tests/journey
uv run pytest -q tests/place

# 변경한 Python 파일의 lint와 format 확인 (파일 목록을 실제 변경에 맞춤)
uv run ruff check src/daengs_backend/services/territory.py tests/test_territory_attempts.py
uv run ruff format --check src/daengs_backend/services/territory.py tests/test_territory_attempts.py

# 공통 변경 등 전체 검증이 필요한 경우
uv run pytest -q
```

전체/Place 테스트에는 Place 추가 의존성이 필요합니다. 개발 환경 준비 명령은 위의
`uv sync --extra place`를 따릅니다. 서버 컨테이너의 동기화는 아래 `ml` 보존 규칙을
따르며, 로컬 테스트 준비를 이유로 운영 컨테이너를 변경하지 않습니다.

### 재실행과 종료 기준

- 선택한 테스트와 필요한 lint가 통과하면, 추가 변경·실패·미해결 영향이 없는 한 같은
  검증을 반복하지 않습니다. 단순한 자신감 확인이나 문서 수정 뒤에 전체 테스트를 다시
  돌리지 않습니다.
- 코드를 추가로 바꿨으면 해당 변경의 영향을 받는 테스트를 다시 실행합니다. 공통부 변경,
  범위 확대, 예상 밖 실패가 생기면 위 표에 따라 대상을 넓힙니다.
- 관련 없는 기존 실패를 발견하면 이번 변경과의 관계를 확인하고 기록합니다. 통과시키기
  위해 테스트를 삭제·skip 처리하거나 별도 기능 수정으로 작업 범위를 임의 확대하지 않습니다.
- `slow` 테스트는 기본 pytest 설정에서 제외됩니다. 실제 모델 로딩 등 그 경로를 바꿨다면
  필요한 테스트 파일에 `uv run pytest -m slow <테스트 경로>`를 적용합니다. 선택 의존성이
  없어 skip된 테스트는 통과로 보고하지 않습니다. 관련 `ml`·`gait`·`screening` 그룹이나
  DB·가중치가 필요한 검증은 해당 하위 지침을 읽고 준비하거나, 실행하지 못한 범위를 밝힙니다.
...
명시하고, 이미 통과한 검사를 이유 없이 재시작하지 않습니다. 머지하는 작업에서는 필요한
검사의 최종 결과를 확인합니다. 이 지침 자체가 머지·배포 권한을 부여하는 것은 아닙니다.

PR의 `## 확인한 것`에는 **선택한 범위의 이유, 실행 명령, 결과, 미검증/skip 범위**를
적습니다. 로컬 결과와 원격 CI 결과는 구분하고, 테스트 개수나 소요 시간을 고정 예산처럼
외우지 말고 해당 실행에서 확인한 값을 보고합니다.
...
- **협업 규칙은 `docs/collaboration.md` 에 있습니다.** 우선순위(P0~P3) · Iteration 기간 · Hold 판단은
  사람이 정합니다. 에이전트는 제안까지만 하고, 작업 단위는 PR 본문의 `## 작업 목록` 을 기준으로 합니다.
