...
0단계 조사 기준은 DEV `8e3d1589`, 현재 목록은 `70a07371` 위의 #530(6단계) 변경을 반영한다. 조사·갱신일은 2026-09-14다.
이 문서는 **남은 구조 변경의 범위를 고정하는 조사 결과**이며 리팩토링 전체 완료 선언이 아니다.
0단계는 조사만 수행했다. 1단계는 B1·B2·B3의 공급 계약/일기 어댑터를 분리하며 DB·공개 계약·정책은 유지한다.

## 먼저 읽을 결과

- [파일별 소유권 표](files.md): 범위 내 708개 파일, 소유 영역·현재 계층·처리·근거·부채.
- [직접 import 판단표](crossings.md): 제품 소스의 교차 영역 관계 362개. 측정 내부 계산/조회 결합 C1은 제거됐다.
- [기계 판독 목록](inventory.json): 위 파일 목록, 1,691개 직접 로컬 import 관계, 판단과 관련 영역.
- [범위 재검사 도구](../../../tools/check_walk_ownership.py): 새 파일·삭제·import 변경·미분류 판단 검출.

범위에 속한 **핵심 Python 254개**, 직접 import 접점 161개, 검증 자산 227개,
SQL 접점 52개, 문자열로 확인한 UI/운영 접점 8개, 명명된 도구 4개, 명시한 통합 접점 2개다.
각 파일은 발견 사유 하나로만 집계한다. 708개를 모두 제품 구현 파일이라고 세지 않는다.
서비스 루트의 `walk.py`·`walk_*.py` **46개는 모두 포함**했다.

362개 관계는 기능 소비·계층 조립·공통 계약을 보존하는 대상으로 분류했다.
...
6단계는 [배경 공급 패키징](../background-package.md)이다. 루트 13개 파일은 동일 공개 객체를 제공하는 호환 경로로 남고, 실제 구현은 `walk_background`가 소유한다. 기록 outbox/재수집 조정은 기록 트랜잭션과 함께 후속 배치한다. 정적 import가 보지 못하는 지연 호환 대상은 [명시 목록](../background-package.json)과 동일 객체 테스트로 별도 검증한다.
...
2. backend의 모든 계층에서 `walk.py`·`walk_*.py`, `services/walk_diary/**`·`walk_generation/**`·`walk_legacy/**`·`walk_artifacts/**`·`walk_background/**`,
