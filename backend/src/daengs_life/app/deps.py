...
# 않는다 — 예열이 도는 중에 `/life/ask` 가 들어오면 두 스레드가 각각 1.2GB 를 따로 올린다. 상주 RAM 이
# 이 카드의 유일한 상시 비용이라(D-021) 그 순간의 2배가 서버를 스왑으로 민다.
#
# **락은 그대로 두고, 기다리는 방식만 바꿨다** (#37). 예열이 도는 중이라면 `/life/ask` 는 락에서
...
def get_encoder() -> Encoder:
    ...
    **`ml` 그룹(torch)이 없으면 503 이다** (D-021). 앱은 그래도 떠야 하므로 lifespan 이 예외를
    삼키고 `/life/ask` 만 503 이 된다 — `Cache` 가 Redis 없이도 앱을 띄우는 것과 같은 태도다.
    ...
    **예열이 도는 중이면 기다리지 않고 503 이다** (#37). 기다린 대가가 나쁘기 때문이다 — 콜드
    ...
    다시 물을 수 있다. `ml` 이 없을 때 `/life/ask` 만 503 이 되는 규칙과 같은 자리다.
...
def warm_up_encoder() -> None:
    ...
    **도는 동안 표시를 세운다** — 그것을 보고 `/life/ask` 가 기다리지 않고 503 을 낸다 (#37).
    ...
    try:
        ...
        logger.warning("임베딩 모델을 못 올렸다 — /life/ask 만 503 이 된다: %s", e.detail)
        return
    except Exception as e:                       # noqa: BLE001 — 예열 실패로 앱을 못 세우면 안 된다
        logger.warning("임베딩 모델을 못 올렸다 — /life/ask 만 503 이 된다: %s", e)
        return
    finally:
        # 여기서 내려야 한다 — 모델이 캐시에 들어간 뒤이므로 다음 `/life/ask` 는 락도 안 잡는다.
...
def warn_if_corpus_uses_another_model(key: str) -> None:
    ...
    **막지는 않는다.** `load.existing_models()` 가 적재 전에 보여 주기만 하는 것과 같은 태도다
    ...
    if not rows:
        # 이 카드를 #34 보다 먼저 머지하면 여기로 온다. `/life/ask` 는 404 만 내지만 **조용히
        # 틀리는 것은 아니다** — 그래서 배포를 막을 이유가 아니라 알려 줄 이유다.
        logger.warning("코퍼스가 비어 있다 — /life/ask 는 404 `근거를 찾지 못했다` 만 낸다")
        return
