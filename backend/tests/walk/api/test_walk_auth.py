...
def test_망가진_토큰이면_401(client: TestClient) -> None:
    ...
    got = client.get(
        "/life/walk-conditions", params=SEOUL, headers={"Authorization": "Bearer not-a-real-token"}
    )
...
def test_권한_없는_role_이면_403(client: TestClient) -> None:
    ...
    got = client.get(
        "/life/walk-conditions", params=SEOUL, headers={"Authorization": f"Bearer {token}"}
    )
