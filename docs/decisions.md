...
`t19` 에서 judge 가 짚은 문장은 **자료 3(`7eb39284`)에 축자로 있습니다.** judge 의 근거도
...
것은 축자 존재와 `t03` 불일치라는 **확인 가능한 사실**입니다. ⚠ 1건이고 남은 5건에서는 judge 가
...
실제로 검토했고**(`judgments_lap1__human.jsonl`, judge 와 일치 6/6), 그것이 이 코퍼스에 달린
...
`t19` 가 실물입니다. 주장이 청크에 **축자로 있는데도** judge 가 *"다른 맥락의 것이라 이 질문에
일반화할 수 없다"* 는 이유로 `unsupported` 에 올렸습니다.
