...
| 카드 키 | 카드를 가리키는 값이 달 정수에서 `catalog.CardSelector = int \| str` 로 넓어졌다. 1~12 는 달, `catalog.KINDS` 의 문자열은 종류다. 저장·로그·API 는 `catalog.card_key()` 가 만드는 문자열(`"4"` · `"strawberry"`)을 쓴다 | #592 (09-18). 앱 경로(`services/ai_card.py` · `routers/ai_card.py`)는 **달 정수를 그대로 넘긴다 — 앱 동작은 안 바뀐다** |
| 과일·채소 카드 (콘솔 전용) | 딸기 **BERRY**(배지 `NEO-S0824`, 부제 `FRUIT DOG`) · 상추 **LETTUCE**(배지 `NEO-0824`, 부제 `VEGGIE DOG`) 둘을 `catalog` 에 넣었다. 틀은 `cardimage/strawberry_template.webp` · `lettuce_template.webp`(994×1582, `backend/tools/cardimage_fruit_templates.py` 로 제목만 지우고 5:8 로 맞춘 것). 본문에 강아지 **얼굴만** 보이는 틀이라 프롬프트 갈래가 하나 더 생겼다 — `engine.build_prompt(face_only=True)`(10월 `face_hidden` 의 반대, 셋은 배타). `DAENGS_CARDIMAGE_MONTHS` 잠금을 보지 않고 **콘솔에서만** 뽑힌다 | #592 (09-18). 틀 채택 `0284c41f`. 실호출 스모크 3장은 잠정 「됨」(상추 아래 띠에 없던 아이콘 둘 — `worklog.md` 09-18 절). **앱 노출은 아직 안 정했다**(`roadmap.md` 4번) |
