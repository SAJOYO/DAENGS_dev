...
요청은 `expected_entries: {entry_id: revision}`와 선택적인 `refresh: false`다.
...
읽는다. bundle은 기존 `walk-storyboard-candidates-v1`이고 실데이터 경로는 synthetic=false다.
...
- 액션 → 이전 산책 대비 이동 변화 → 세션 속도 변화 → 빈 거리 보충 순서다.
