...
다시 돌린다 — 공공데이터를 다시 받지 않는다.
    ...
    python -m daengs_place.ingest restrictions             # 아직 파생 안 된 행만
    python -m daengs_place.ingest restrictions --all       # 전량 재파생 (표를 고친 뒤)
    python -m daengs_place.ingest restrictions --source kcisa
...
from daengs_place.place.restriction_map import (
