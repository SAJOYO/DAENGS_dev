...
| [D-054](#d-054) | LangSmith 트레이싱은 기본 꺼짐의 명시적 옵트인 — D-037 을 뒤집지 않고 예외를 판다 | 2026-09-05 |
...
## D-054
### LangSmith 트레이싱은 기본 꺼짐의 명시적 옵트인 — D-037 을 뒤집지 않고 예외를 판다
...
`request_id` → `chat_turns` → `chat_sessions` 로 우리 DB 안에 이미 있습니다. 제3자에게
보내서 얻는 것이 0 인데 보내면 그건 유출 표면일 뿐입니다.
...
| 별도 저장 | 우리 DB 가 아닌 LangSmith. 운영 로그·메트릭에는 아무 변화가 없습니다 |
| 보존 기한 | base trace 14일 (자동 만료) |
| — | 마스킹 (`core/tracing.py` `scrub_payload`) |

#### 선행 조건 — 켜기 전에

**개인정보처리방침의 제3자 제공 목록에 LangChain 을 올리는 것이 먼저입니다.**
대화 내용을 국외 사업자에게 보내는 것이라, 출시 점검의 "방침 제3자 목록" 항목과 같은
자리입니다. 코드가 먼저 들어가는 것은 괜찮습니다 — 꺼져 있으면 아무것도 안 나갑니다.

리전은 `LANGSMITH_ENDPOINT` 로 고릅니다. GCP APAC 이 전 요금제에서 열려 있습니다.
