...
**작성 2026-09-11 (#435) · 결정 번호 D-068 · 2026-09-11 구현·배포 완료.**

`daengs_life/realtime/`(산책 적합도 · 과거 날씨)을 **Cloud Run 서비스로 뗀 설계와 실측**이다. 여기에는
**설계와 결정 근거, 그리고 배포 뒤 실측**을 둔다 — `corpus-pipeline.md`(D-062)와 같은 자리이고, 명령
절차는 `runbook.md` 「실시간 서비스 (GCP)」 절에 있다.

✅ **§7 의 검증 일곱을 전부 실측으로 채웠다** (2026-09-11). 서비스는 `daengs-realtime`
(asia-northeast3, 리비전 `daengs-realtime-00002-tp7`) 로 돈다 —
`https://daengs-realtime-584617819762.asia-northeast3.run.app`. 아래는 그 실측이다.
...
6379 는 **규칙을 만들지 않는다 — `default-allow-internal` 이 이미 덮는다(2026-09-11 실측).**

규칙 없이 Cloud Run 잡(`corpus-pipeline` SA, `--network=default --subnet=default`)에서 VM 의
`10.178.0.2:6379` 에 붙였더니 `-NOAUTH Authentication required` 가 돌아왔다 — 연결 자체는
성공했고 인증만 남은 응답이라, 그 앞을 막는 규칙이 없다는 뜻이다. `default-allow-internal` 이
`10.128.0.0/9` 에서 tcp 전 포트를 허용하고 서브넷 `default`(`10.178.0.0/20`)가 그 안에 들어서
이미 덮인다.

**5432(Postgres) 쪽에만 `allow-pg-from-run` 이 따로 있는 이유는 6379 가 막혀서가 아니다** — 그
규칙이 이 기본 규칙보다 먼저 만들어졌고 `pipeline.sh` 가 이미 관리하고 있기 때문이다. 6379 를
위해 새 규칙을 만들 이유가 없고, `infra/gcp/realtime.sh` 도 만들지 않는다. 출처가 서브넷 밖으로
안 나가므로 **인터넷에서는 여전히 못 닿는다** — D-042 의 *"6379 를 인터넷에 안 연다"* 가 그대로
지켜진다.
...
**전부 2026-09-11 실측으로 닫혔다.** 아래는 그 증거다.

| # | 확인할 것 | 결과 |
| --- | --- | --- |
| ① | Cloud Run **서비스**(잡이 아니라)에서 Direct VPC egress 가 되나 | ✅ 된다 — 서비스가 `10.178.0.2:6379` 에 실제로 붙어 `-NOAUTH Authentication required` 를 받았다(§4). 방화벽 규칙 없이도 됐다 |
| ② | 그 인스턴스가 `10.178.0.2:6379` 에 실제로 닿나 | ✅ 닿는다 — ①과 같은 근거에 더해, 부산(35.1796, 129.0756) 새 격자 호출 **전후로 VM Redis 의 `rt:budget:datagokr-vilage-fcst:20260911` 카운터가 6 → 8 로 늘고** `rt:kma-vilage-fcst:ultra:98,76` · `village:98,76` 키가 새로 생겼다 — **VM 의 backend 와 같은 Redis 를 공유한다는 직접 증거** |
| ③ | 이미지 크기와 콜드 스타트 실측 | 이미지 **445MB**(파이프라인 CPU 이미지 2.3GB 대비 — 확정). 콜드 스타트는 **못 쟀다** — 배포 직후 헬스체크가 인스턴스를 띄워 격리가 안 됐다. 재려면 15분 이상 인스턴스를 놀린 뒤 첫 호출을 재야 한다. 추정치는 안 적는다 |
| ④ | 503 본문이 프록시를 통과해 그대로 오나 (콘솔 패널) | ✅ — 단위 테스트로 확인했다(`backend/tests/test_realtime_service_split.py::test_client_keeps_status_and_body_untouched_on_503` 등). 실 배포에서는 판정 불가(503)를 일부러 유발해 보지 않았다 — 2026-09-11 라이브 호출은 전부 200(캐시된 격자 강남 1.34초 · 새 격자 부산 7.16초) |
| ⑤ | VM 의 backend 가 ID 토큰으로 서비스를 부를 수 있나 | ✅ 된다 — VM 계정 토큰으로 `/health` 가 **200 · 0.11초**, `/life/walk-conditions` 실제 판정도 **200**(강남 1.34초 · 부산 7.16초, 둘 다 8초 예산 안). 무인증 호출은 **403**으로 막힌다 |
| ⑥ | `DAENGS_REALTIME_URL` 을 지우면(또는 비워 두면) 옛 경로로 정확히 돌아오나 | ✅ — 단위 테스트(`test_default_registration_still_uses_the_in_process_router` · `test_realtime_branch_registers_with_the_same_auth_as_the_default_branch`, 값 있음/없음 두 번 reload)로 코드 레벨 확인. VM 은 지금 이 값이 `.env` 에 들어간 채 **비활성**이다 — 컨테이너가 재생성되지 않아 그 값을 모르고 47시간째 그대로 돌며 앱 API 는 여전히 200 이다(runbook §「실시간 서비스 (GCP)」) — 즉 "값이 안 먹으면 옛 경로 그대로"가 실제로도 확인됐다 |
| ⑦ | **스모크 — 이미지를 실제로 띄워 두 엔드포인트를 실제로 부른다** | ✅ — 로컬이 아니라 **배포된 서비스 자체**로 더 강하게 확인했다. `/life/walk-conditions` 를 캐시된 격자(강남)·새 격자(부산) 둘 다 실제로 호출해 200 을 받았고, 새 격자 호출은 Redis 에 새 키를 만드는 것까지 이어졌다(②) — `--only-group realtime` 이 패키지를 놓쳤다면 이 경로에서 import 가 죽었을 것이다 |
