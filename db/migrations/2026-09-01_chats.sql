-- Still-unapplied chat migration. This intentionally creates the final design directly;
-- there is no ALTER migration for the discarded per-message draft schema.
-- Re-runnable because this repository has no migration version table.
...
CREATE UNIQUE INDEX IF NOT EXISTS chat_sessions_one_draft_idx
    ON chat_sessions (app_user_id, pet_id) WHERE last_message_at IS NULL;
...
CREATE TABLE IF NOT EXISTS chat_turns (
    ...
    CONSTRAINT chat_turns_state_check CHECK (
        ...
        OR (processing_status = 'completed' AND assistant_content IS NOT NULL
        ...
        OR (processing_status = 'failed' AND assistant_content IS NULL
            AND assistant_status IS NULL AND public_response IS NULL
...
CREATE TABLE IF NOT EXISTS chat_summaries (
    ...
    CONSTRAINT chat_summaries_state_check CHECK (
        ...
        OR (processing_status = 'completed' AND title IS NOT NULL
        ...
        OR (processing_status = 'failed' AND title IS NULL
            AND question_summary IS NULL AND answer_summary IS NULL
            AND key_points IS NULL AND cautions IS NULL AND source_citations IS NULL
