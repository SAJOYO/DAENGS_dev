...
- `walk_context_backfill`: 기록별 누락 관측의 선택·재수집 예약과 CAS. 카탈로그 및 공급 해시를 소비한다. 7단계 #531에서 outbox와 함께 `walk_records.backfill`로 배치한다.
...
전체 저장소 테스트, 실제 공급자·모델 호출, 실기기 검증은 이번 범위에 포함하지 않는다. 기록/사진은 후속 7단계 #531에서 다룬다. 원본/봉인, 측정/공간 조회의 패키지 재배치와 최종 통합 검증은 그 이후 단계다.
