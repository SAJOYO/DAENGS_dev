...
현재 루브릭은 `facility-judge-ko-v3`, 기본 대조 사례는 `anchors.v3.jsonl`의 22개다. 요청 수행·범위 선택·결과 설명을 독립적으로 평가한다. 명확한 요청을 수행하지 못한 것은 의도 실패지만, 실제 미실행과 일치하는 실패 안내나 완료 주장이 없는 되묻기는 결과 설명 축에서 통과할 수 있다. 질문에 거짓 완료 전제가 있거나, 실제로 실행했는데 못 했다고 말하면 실패다.

결과 설명 축에는 실행 전후 상태·실행 기록·실제 표시 문구만 제공한다. 사용자 요청·이전 대화·모델 계획을 제외하고 요청 수행 정책도 별도로 분리해 중복 감점을 줄인다. 요청과의 일치는 나머지 두 축이 확인한다. 문구의 진위를 이 관측만으로 판단할 수 없다면 `uncertain`이며, 이 축만으로 대화 전체의 품질을 보증하지 않는다.
