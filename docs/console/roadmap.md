...
| 회원 · 반려견 | `read` | `/console/users` — 닉네임·이메일·카카오 회원번호 검색 · **훑는 목록**(`admin:manage`) · 마스킹 상세 · 반려견 · 원문 보기(`pii:read`) · 정지/해제(`ops:write`) | `/admin/app-users`(READ · **/list ADMIN_MANAGE** · PII_READ · OPS_WRITE) | ✅ #211 · #212 · #257 | **이메일 검색은 이 앱키로는 영영 0건이다** — 이메일 동의를 못 받아 `email_hash` 가 전 회원 NULL. 오늘 도는 것은 닉네임과 목록이다. 화면이 그것을 안내한다 |
...
| A2c | **회원 목록** — 검색 말고 훑어보는 길. `GET /admin/app-users/list`(`ADMIN_MANAGE`) + 화면 안의 목록 | #211 이 일부러 안 만들었다 — `/admin/app-users` 가 `Perm.READ` 라 목록을 열면 VIEWER 까지 전 회원을 넘겨보게 된다. "메일 보낸 사람 찾기"와 "회원 훑기"는 다른 일이다 | 사람 결정(권한) ✅ 2026-09-05 | XS | ✅ #257. **목록 줄에 개인정보가 한 칸도 없다**(마스킹한 것조차) — 그래서 §7 "역할 발급" 을 안 기다렸다. 찾기는 `read` 그대로다 |
