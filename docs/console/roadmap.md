...
| A4-1 | **감사 로그 조회 화면** `/console/audit` — 로그인 시도 · 계정 변경 · (뒤에) 복호화 조회 | #203 이 테이블만 만들었고 보는 길이 psql 뿐이다. #207 이 기록을 늘려 볼 것이 생겼다 | #203 · #207 | S | ⬜ |
...
| A2b | **원문 복호화와 상태 변경** — `pii:read` 원문 보기 + `admin.app_user.pii_revealed` 감사 · `PATCH status` + 세션 끊기 | 감사에 남길 가치가 있는 것은 "가려서 봤다"가 아니라 "원문을 열어 봤다" — 그 선에서 A2 를 갈랐다 | A4 ✅ · A3 ✅ · **#211** · 복호화는 `core/crypto.py` 만 (D-012) | S | ⬜ #212 |
