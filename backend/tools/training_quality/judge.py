...
**후** 신뢰"* 라고 못 박았다. Life 쪽이 지금 라벨 4개에서 멈춰 있다 (`RAG-075`).
...
def agreement(reference: list[Judgment], candidate: list[Judgment]) -> dict[str, Any]:
    ...
    **사람 라벨도 판정 파일로 적는다** (`judge_model: human` · `prompt_version: 0`). 파일 모양을
    같게 둔 이유가 이것이다 — 사람↔judge 를 견주는 코드와 judge↔judge 를 견주는 코드가 갈리면
    프롬프트를 고칠 때마다 둘 중 하나가 낡는다 (#305 · `RAG-075` ①).
