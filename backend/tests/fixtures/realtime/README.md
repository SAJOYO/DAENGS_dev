...
**테스트 입력**이고 인덱싱되지 않으며 답변으로 나가지도 않는다 (RAG-012 · RT-001 검문소 B 주석).
...
- **`PTY` 5·6·7** — 실측은 여전히 `'0'` 뿐이다. 다만 **파서는 이미 받는다** — RT-004 가 활용가이드
  근거로 0~7 을 넣었고 8 이상만 실패한다. 여기서 기다리는 것은 그 셋의 **실물**이다
