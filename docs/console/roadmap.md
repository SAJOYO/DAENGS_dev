...
| A3 | **관리자 계정 관리** — `GET/POST /admin/admins` · `PATCH`(role · status) + `/console/admins`. `seed-admin` 은 최초 1회로 남겼다 | D-014 5단계가 정의만이었다. 이제 `pii:read` 를 못 가진 계정을 실제로 만들 수 있어 A2 의 가리기가 검증된다 | A4 ✅ | S | ✅ #207 (DB 변경 없음 — `admin_users` 는 두 DB 에 이미 있다) |
