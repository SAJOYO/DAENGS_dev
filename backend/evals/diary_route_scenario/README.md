...
아래 초기 실험·public-03 측정·미구현 판단은 **각 실행 당시 기록**이다.
현재 정책에서 과거 결과를 덮거나 기존 verify의 hash 불일치를 결과 수정으로 맞추지 않는다.

2026-09-14, DEV 기준 **154cc610**, PR #508. 초기 실험은 운영 코드·프롬프트·슬롯 정책을 변경하지 않았다.
당시의 [LLM 입력 정규화](LLM_INPUT.md) v1은 카드 작성 경로·프롬프트만 바꾸고 슬롯 정책은 유지했다.
