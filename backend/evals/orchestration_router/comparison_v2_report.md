...
**2026-09-06 · LangGraph 유지, `agent/` 는 조건부 보존** — 자세히는 D-055 ⑦.

- 운영 기본값은 `DAENGS_ORCHESTRATOR=langgraph` 그대로. 정확도는 동률 범위이고 비용은 에이전트가
  약 두 배라, 바꿀 근거가 없다.
- 에이전트 코드는 지우지 않는다. 후속 세 카드(실사용 홀드아웃 · 스코프드 리다이렉트 · 라우팅 정책 ·
  RAG · 컨텍스트) 가 끝난 뒤 다시 평가하고, 그때까지 "툴 결과를 보고 다음 수를 정하는 선택" 이
  필요한 카드가 없었으면 그때 지운다.
- 보존 규칙: 라우터 경계를 바꾸면 같은 PR 에서 에이전트 프롬프트도 바꾼다. CI 가 `agent` extra 를
  깔아 에이전트 테스트를 돌린다.
- "답할 수 없다" 민원은 두 구현이 똑같이 지키는 라우팅 정책(v5 경계)에서 오는 것이지 프레임워크의
  한계가 아니다. 그것은 위 후속 카드의 일이다.
