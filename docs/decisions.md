...
### ③ 무엇을 안 하나
"고양이 카페를 찾아 주는 것이 그렇게 나쁜가" 는 물어볼 만한 질문이지만, 이 결정은 **한 서비스가
무엇에 대해 말하는 서비스인지**를 고른 것입니다. 종을 가리지 않는 장소 검색을 하고 싶어지면
그것은 Place 능력의 범위를 넓히는 별도 결정이고, 라우터를 흐리게 두는 방식으로는 하지 않습니다.
