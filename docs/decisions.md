...
**거기서 멈췄습니다.** 남은 둘(`t07`·`t19`)도 표현 트집으로 보이지만 그것을 "트집" 이라고
부르는 것은 지금 **제 읽기**입니다 — `RAG-075` ② 가 정확히 그 실패였고, 사람이 라벨링해 보니
judge 가 맞았습니다. 한 랩에 맞춰 계속 고치면 그 랩에서만 잘 맞는 채점자가 됩니다.

⚠ **judge 가 엄한 쪽으로 기웁니다.** 21건 중 6건을 걸었고 그중 둘이 표현 트집에 가깝습니다.
`RAG-075` ③ 이 Life RAG 에서 관찰한 것과 **같은 방향**입니다 — 축도 코퍼스도 모델도 다른데
기운 방향이 같습니다. 사람 라벨이 가릴 문제이지 프롬프트로 더 밀 문제가 아닙니다.
...
### ⑦ 사람 라벨을 안 하기로 했습니다 — 그래서 승격 경로가 닫혔습니다
2026-09-07, 사람이 이 코퍼스에 라벨을 달지 않기로 정했습니다. `RAG-007` 이 요구한 30개가
**생기지 않습니다.**
