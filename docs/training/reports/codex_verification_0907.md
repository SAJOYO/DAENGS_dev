...
| 3 | D-060 ⑧ prompt gap claim | CORRECT | `backend/tools/training_quality/judge.py:97-136`; `docs/decisions.md:3543-3549` |
...
| D-060 ⑧: prompt는 world truth를 막지만 answer appropriateness는 막지 않음 | CORRECT | prompt는 `세상에서 사실인지는 묻지 않는다`고 명시하고 (`backend/tools/training_quality/judge.py:97-104`), 나머지 규칙과 결론부 어디에도 “이 질문에 적절한 답인지 묻지 않는다”는 금지가 없다 (`backend/tools/training_quality/judge.py:105-136`). D-060이 지적한 바로 그 차이는 `docs/decisions.md:3543-3549`. |
...
| ③ | CORRECT | 두 패키지가 같은 env 이름을 각자 읽고 키를 명시 전달한다는 설명 (`docs/decisions.md:3411-3419`)은 두 설정 (`backend/src/daengs_life/rag/core/config.py:95-116`; `backend/src/daengs_backend/config.py:161-195`)과 두 client (`backend/src/daengs_life/rag/stages/judge.py:137-151`; `backend/tools/training_quality/judge.py:185-201`)에 맞다. |
| ④ | CORRECT | D-060은 training faithfulness와 Life `answers_question`을 분리한다 (`docs/decisions.md:3425-3449`). RAG-074도 `answers_question`이 질문과 답변만 본다고 규정한다 (`docs/life/decisions-rag.md:8591-8603`), 반면 training prompt는 자료 청크 지지만 묻는다 (`backend/tools/training_quality/judge.py:92-114`). |
| ⑤ | CORRECT | D-060은 ANSWER만 채점한다고 한다 (`docs/decisions.md:3455-3459`). RAG-074도 abstain/refuse 경계 문항을 제외한다 (`docs/life/decisions-rag.md:8674-8678`), training 구현도 `decision != "ANSWER"`를 제외한다고 명시한다 (`backend/tools/training_quality/judge.py:268-270`). |
...
| ⑧ | WRONG | prompt gap에 대한 설명 자체는 맞지만 (`docs/decisions.md:3543-3549`; `backend/tools/training_quality/judge.py:97-136`), 절의 전제인 t19 문장이 청크에 `축자로` 있다는 주장은 틀리다. 실제 두 문장은 서로 다르다 (`backend/evals/training_quality/judgments_lap1.jsonl:20`; `backend/evals/training_quality/answers_lap1.jsonl:20`). |
