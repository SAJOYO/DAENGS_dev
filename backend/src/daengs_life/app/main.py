...
엔드포인트를 여기 직접 붙이지 않는다: 파트①의 `/life/ask` 가 들어올 때 이 파일에서 겹치는 것이
...
async def lifespan(_: FastAPI):
    ...
    **연결이 안 돼도 앱은 뜬다** — `Cache()` 가 스스로 메모리로 떨어진다. 캐시는 가속이지
    ...
    # **실패해도 앱은 뜬다** — `ml` 그룹(torch)이 없는 환경에서도 `/life/walk-conditions` 는 돌아야 하고,
    # 캐시가 Redis 없이 뜨는 것과 같은 태도다. 그 경우 `/life/ask` 만 503 이 된다.
    #
    # **여기서는 동기로 부른다.** 이 앱은 `/life/ask` 전용에 가까워서 모델이 올라오기 전에 뜰 이유가
    # 없다. 로그인·`/life/walk-conditions` 와 한 프로세스인 `daengs_backend` 쪽은 그럴 이유가 있어서
