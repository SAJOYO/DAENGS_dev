...
- 내부 작업은 원자료·버전·캐시 의존성을 보존한다. 실제 모델 요청은 `diary-prose-input-v5`로 별도 정규화하며 원 좌표·원자료 ID·해시·진단값·이동 시각 숫자를 보내지 않는다. [활동 재료 조립](diary-movement-materials.md)은 카드 범위에 맞춰 동선과 속도를 결합한다.
