...
### 보행·스크리닝 **생성**도 구성원이 연다 (Task 12)

위 표에서 `repositories/gait_record.py` 가 "구성원" 인 것은 처음엔 **읽기**뿐이었다. 새 기록을
여는 `services/gait.py::start_analysis` 와 `services/screening.py::start_record` 가 `get_owned`
를 쓰던 동안은 **돌보미가 보행 영상도 스크리닝 사진도 못 올렸다** — 결정 ②("돌보미는 기록하고
본다")와 어긋나는 자리라고 알고 남겨 뒀던 후속이, 최종 리뷰 뒤 이 브랜치에서 마저 열렸다. 지금은
둘 다 `pet_repo.get_accessible` 이다 — 구성원(대표 ∪ 돌보미)이면 새 기록을 열 수 있다.
...
**스크리닝은 이유가 하나 더 있다.** 밥·약과 성질이 다른 것은 여전하다 — 케어 로그는 행
하나지만 이쪽은 **저장소 객체(영상·사진)를 만든다.** 업로드 티켓·키 발급·고아 객체 청소가
전부 "누가 올렸나" 에 매달려 있다. 열면서 확인한 것 — 티켓 키(`build_screening_photo_key` 가
여전히 만든 사람의 id 로 키를 만든다, 접근 판정과는 별개 문제라 그대로 둔다), 탈퇴·강아지
삭제의 파기 경로(`gait_service.cleanup_for_pets` 는 `pet_id` 로 걸려 있어 돌보미가 만든 것도
같이 지워진다. `screening_service.cleanup_for_owner` 는 **만든 사람** 기준이라 그 사람 자신이
탈퇴할 때는 맞게 지워지지만, 대표가 탈퇴해도 돌보미가 만든 스크리닝 기록은 안 지워진다 —
다만 대표는 돌보미가 남아 있으면 애초에 탈퇴가 막히므로(§3 탈퇴 가드) 지금은 사고가 안 난다).

⚠️ **스크리닝은 "본다" 의 절반이 아직 안 열렸다.** `gait_records` 는 `pet_id → pets.app_user_id`
로 소유가 유도되어 돌보미가 만든 기록도 대표가 읽는다. `screening_records` 는 소유가 만든
사람(`app_user_id`)에 그대로 저장되고 조회·확정·삭제가 전부 그 값으로만 걸린다 — 애초에
`pet_repo.member_condition` 을 한 번도 쓴 적이 없는 레포지토리다. 그래서 돌보미가 연 스크리닝
기록은 **대표에게도 안 보인다.** 넓히려면 `repositories/screening.py` 를 구성원 기준으로 다시
짜는 별도 변경이 필요하고, 그것은 이번에 안 열었다 — 제품 결정이 먼저다.
...
- **초대 수락은 일회용이다** — §3 "수락 — 검증 순서" 뒤에 적은 대로, 응답을 놓친 재시도는
  ...
  다시 확인해야 한다.
- **보행·스크리닝의 "새 기록" 버튼은 돌보미 화면에도 이제 뜬다** (Task 12) — 생성이
  `get_accessible`(구성원)로 열렸다(§2 "보행·스크리닝 **생성**도 구성원이 연다"). `is_owner`
  로 가려야 하는 것은 여전히 수정·배웅·삭제·사진(강아지 프로필)과, 보행의 확정·삭제(개별
  기록)뿐이다 — 그 자리에서 누르면 대표만 허용된 자리라 404 다. **스크리닝은 예외가 하나
  있다** — 돌보미가 올린 기록은 대표를 포함해 **만든 사람 자신에게만** 보인다(위 §2 의 "본다
  의 절반" 경고). 앱이 "우리 강아지 스크리닝 이력"처럼 구성원 전체의 기록을 한 화면에
  모아 보여주면 안 된다 — 지금은 그렇게 안 된다.
...
- **스크리닝의 읽기를 구성원 기준으로 넓힐 것인가** — 생성은 Task 12 에서 열렸지만
  (§2 "보행·스크리닝 **생성**도 구성원이 연다"), 조회·확정·삭제는 여전히 만든 사람
  기준이라 돌보미가 연 기록이 대표에게 안 보인다. `repositories/screening.py` 를
  `gait_record.py` 처럼 `_owned`/`_accessible` 두 바닥으로 다시 짜는 별도 변경이 필요하다
  (제품 결정 먼저).
