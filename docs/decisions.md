...
마운트·비용 3배)와 비용 계산은 노션 "클라우드 이전 검토" 문서에 있습니다. 절차는
`docs/deploy-gcp.md`, GCP 전용 구성은 `docker-compose.gcp.yml` + `nginx/gcp.conf`.

- **배포 소스는 `main` 브랜치**입니다 (PR #114 가 첫 스냅샷). 별도 배포 레포를 만들지
  않습니다 — 정본이 둘이 되면 핫픽스가 갈라지고(D-032 가 피한 그 상황), 같은 목적을
  기존 규칙(완성 단위마다 dev → main PR)이 이미 제공합니다.
- **서빙만 옮깁니다.** 크롤러·코퍼스 정본은 로컬 서버에 남습니다 — 코퍼스 raw 는
  ...
  생깁니다. GCP 유지가 확정되면 그때 2차로 이전합니다 (deploy-gcp.md "2차").
- **DB 는 dev/prod 로 갈라집니다.** GCP 3대(덤프 복원)가 운영 정본, 로컬 서버 DB 는
  개발용으로 남습니다. 그래서 GCP 는 5432/6379 를 인터넷에 열지 않습니다 — 지금
  compose 의 LAN 개방을 인터넷에 재현하지 않습니다.
