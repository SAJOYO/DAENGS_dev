...
# 강수량은 **같은 이름의 도메인이 흔들린다.** `RN1` 이 실황에서는 `'0'`(숫자), 예보에서는
# `'강수없음'`(구간)으로 오고 — **오퍼레이션 안에서도 흔들린다**: 2026-09-10 실측에서 단기예보
# `PCP` 가 1~2일차에는 `'강수없음'` 인데 **3일차부터 `'0'`** 이었다 (리드타임이 길면 숫자로 온다).
# 그래서 **넓은 쪽(`Interval`)으로 통일**하고 점은 폭 0 구간으로 올린다 (②-d-3 결정 1).
# 좁은 쪽으로 맞추면 `'1mm 미만'` 을 임의 수치로 눌러야 한다.
...
def parse_interval(raw: str) -> Interval:
    ...
    **모르는 문자열은 조용히 0 으로 떨어지지 않고 실패한다** (②-d-1). 조용히 0 이 되면 비 오는 날
    "강수 없음"이라고 답하고도 아무 신호가 없다 — 그게 이 판정에서 가장 위험한 실패다.
    `ValueError` 로 멈추면 `/life/walk-conditions` 가 `unknown`(503)으로 나가고 로그에 원문이 남는다.

    RT-004 에서 `-`·빈 값을 "없음"으로 받게 했다 (`_NONE` 의 주석).

    ✅ **`E1` 은 2026-09-10 에 닫혔다 — 기다리던 문자열이 존재하지 않았다** (RAG-085 ⑤).
    전국 16격자 × 단기예보 3일치 + **과거 발표분 7일치**에서 `PCP`·`SNO` 표기를 전부 모아 여기에
    먹였다: **33종 전부 통과, 실패 0.** 실제로 오는 모양은 다섯이다 —
    `'강수없음'`/`'적설없음'`/`'0'` · **`'1mm 미만'`**(문서는 `1.0mm 미만` 이라 적는다) ·
    `'1.0mm'` 처럼 단위 붙은 점값 · `'0.1'`~`'3.8'` 맨 소수 · `'1'`~`'3'` 맨 정수.

    🔴 **`'1.0~29.9mm'` 같은 구간 표기는 안 온다.** 1~30mm 사이는 구간이 아니라 **점값**이고,
    `_RANGE` 는 7일 실측에서 한 번도 안 걸렸다 — 기상청 활용가이드의 범주 표기와 실제 응답이 다르다.
    ⚠ 다만 `_OVER`(`'50.0mm 이상'`)와 `_RANGE` 는 **여전히 미검증**이다. 그 7일에 폭우가 없었다.
    점값으로 오는 것이 확인됐으니 35mm 도 `'35.0'` 으로 와서 `_POINT` 가 받을 것 같지만 **추론이다.**
    """
    text = (raw or "").strip()
    if _NONE.match(text):
