"""Historical public names; implementation belongs to the records/photo owner package."""

from importlib import import_module

_EXPORTS = {
    "reserve": "daengs_backend.services.walk_records.context",
    "EntryConflict": "daengs_backend.services.walk_records.errors",
    "EntryInvalid": "daengs_backend.services.walk_records.errors",
    "EntryNotFound": "daengs_backend.services.walk_records.errors",
    "guard_v1": "daengs_backend.services.walk_records.policy",
    "build_profile": "daengs_backend.services.walk_records.profile",
    "response": "daengs_backend.services.walk_records.v1",
    "list_entries": "daengs_backend.services.walk_records.v1",
    "apply_change": "daengs_backend.services.walk_records.v1",
    "write": "daengs_backend.services.walk_records.v1",
    "remove": "daengs_backend.services.walk_records.v1",
    "profile": "daengs_backend.services.walk_records.v1",
}
__all__ = list(_EXPORTS)


def __getattr__(name):
    if name not in _EXPORTS:
        raise AttributeError(f"module {__name__!r} has no attribute {name!r}")
    value = getattr(import_module(_EXPORTS[name]), name)
    globals()[name] = value
    return value


def __dir__():
    return sorted(set(globals()) | set(__all__))
