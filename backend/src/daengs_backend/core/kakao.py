...
class KakaoIdentity:
    ...
    **앱이 요청 본문에 같이 실어 보낸 값은 쓰지 마세요** — 닉네임이든 이메일이든 앱이
    지어낼 수 있습니다. 여기 있는 것은 전부 카카오가 서명한 토큰 **안**에서 꺼낸 것이라
    위조할 수 없습니다. 둘은 이름이 같아도 다른 값입니다.
    """
    ...
    nonce: str | None
    #: 카카오 프로필 닉네임. **동의를 안 받았으면 None 입니다.**
    #:
    #: 이메일과 **등급이 다릅니다** — 이메일·전화번호·이름은 비즈 앱 심사(사업자 등록)가
    #: 있어야 받을 수 있지만, 닉네임은 프로필 기본 항목이라 개발자 앱에서도 선택 동의로
    #: 열립니다. 지금 우리 앱키는 앞의 것들을 못 받는 상태라, 이 값이 카카오에서 들어오는
    #: **유일한 사람 이름**일 수 있습니다.
    #:
    #: `services/app_auth.py` 가 첫 가입 때 닉네임의 씨앗으로 씁니다. 없으면 거기서
    #: `댕댕이` + 코드로 짓습니다 — 이 값이 없다고 로그인이 막히지는 않습니다.
    #:
    #: **기본값을 두고 맨 뒤에 둔 것은 일부러입니다.** 이 클래스를 만드는 자리가 코드와
    #: 테스트에 여럿이라, 필수 인자로 끼워 넣으면 닉네임과 무관한 곳까지 다 고쳐야 합니다.
...
async def verify_id_token(
    ...
    return KakaoIdentity(
        ...
        # **이건 id_token 안의 값입니다.** 위 클래스 주석이 "앱이 같이 보낸 닉네임은
        # 쓰지 마세요" 라고 한 것은 **요청 본문으로 온 값** 이야기입니다 — 그건 앱이
        # 지어낼 수 있지만, 여기 것은 카카오가 서명한 토큰 안에 들어 있어 위조할 수
        # 없습니다. 공백만 있는 닉네임은 없는 것으로 봅니다.
        nickname=(claims.get("nickname") or "").strip() or None,
