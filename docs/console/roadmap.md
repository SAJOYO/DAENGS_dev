...
| 사람들이 무엇을 묻나 | 🔵 **대화 집계는 콘솔** (#223) — 능력별 분포 · 응답 결과 8값 · 처리 실패. **아직 볼 값이 없다** (개발 DB `chat_turns` 0건) | 요청 메타데이터(지연 · request_id)까지 | B2 |
| 크롤이 실패했다 | ✅ 이미 콘솔 (#76) | — | — |
...
| B4 | `운영 지표` 화면 `/console/metrics` — B2 · B3 를 한 화면에 | 준비 중 카드 | B2 or B3 | S | 🔵 #223 이 첫 판을 냈다. 남은 것은 **B2 를 그 화면에 붙이는 일**뿐 |
