...
공동 돌봄으로 **함께 보는 것**과 **실행한 사람만의 것**을 가른다. 아래 §2 접근 판정 표는 **지금 구현**을 적은 것이고, 이 절은 **제품 계약**이다. 둘이 다른 자리는 표 아래에 따로 적었다.

| 구분 | 기능 | 지금 상태 |
| --- | --- | --- |
| **공유** | 밥 · 약 · 간식 케어 기록 | 구현 — 현재 구성원이 논리 강아지 전체를 본다 |
| **공유** | 산책 기록 | 구현 — `GET /app/pet-walks`, `GET /app/pets/{id}/walks[/{walk_id}]` |
| **공유 (계약)** | 진료비 영수증 | **미구현 — 고도화.** 아래 참고 |
| 비공유 (실행자 개인 기록) | 보행 분석 · 피부 검사 | MVP 에서 공유하지 않는다. 공유는 고도화 방안이다 |
| 비공유 | 챗봇 대화 · 카드 뽑기 · 산책 모아보기 · 산책 게임 | — |

- **개인 기록은 나가기 · 내보내기와 무관하게** 실행한 사람이 계속 소유한다.
- **나가기 · 내보내기가 끊는 것은 공유 기록의 조회(와 케어 기록 삭제 자격)뿐**이다. 기록을 지우거나 옮기지 않는다 (「퇴장 · 내보내기」).

### 지금 구현과 계약이 다른 자리 (코드 변경 보류)

| 기능 | 지금 구현 | 계약 |
| --- | --- | --- |
| 보행 분석 | 강아지의 구성원에게 조회 · 생성이 열려 있다 (`repositories/gait_record.py::_accessible`, 아래 「보행 생성은 구성원이 연다」) | 실행자 개인 기록 |
| 피부 검사 | 강아지에 붙은 기록은 구성원에게 조회 · 생성이 열려 있다 (`repositories/screening.py::get_accessible`, 아래 「스크리닝도 이제 구성원이 연다」) | 실행자 개인 기록 |
| 진료비 영수증 | **공유되지 않는다.** 모든 조회가 행 소유자(`pet_repo.get_owned`) + 올린 사람(`VetVisit.app_user_id`) 기준이다. 연결 없이 참여한 돌보미는 함께 돌보는 아이에서 목록 · 찍기가 404 다 | 공유 |

진료비 영수증의 현재 동작과 고도화 방안은 `docs/vet-visits.md` 「공동 돌봄」에 있다.
...
| `DELETE /app/pets/{pet_id}/members/{user_id}` | 그룹 주보호자 또는 본인 — `pet_id` 는 **부른 사람의 카드 id**(`display_pet_id`). 갈래는 아래 「퇴장 · 내보내기」 표 |
...
`DELETE /app/pets/{petId}/members/{targetAppUserId}` 하나로 둘 다 한다 — 그룹 주보호자가
부르면 내보내기, 본인이 부르면 나가기.

⚠️ **`petId` 는 부른 사람의 카드 id(`display_pet_id`)다.** 앱이 아는 것은 그것뿐이고, 연결한
공동 보호자에게 그 id 는 **자기가 대표인 자기 행**이다. 그룹의 앵커 행 id 는 앱에 안 내려간다.
그래서 판단은 전부 **논리 그룹** 기준이고, 요청한 행의 대표가 누구인지로는 아무것도 안 정한다.
(행 대표로 정하던 동안 연결한 공동 보호자의 나가기가 409 로 막혀 **나갈 방법이 아예 없었다.**)

| 부른 사람 | 대상 | 결과 |
| --- | --- | --- |
| 그룹 주보호자 | 그룹의 다른 보호자 | **204** — 내보내기 |
| 공동 보호자 (연결했든 안 했든) | 자기 자신 | **204** — 나가기 |
| **그룹 주보호자가 아닌 누구든** | 남 (그룹 주보호자를 지목한 것도 포함) | **403** 「다른 보호자를 내보낼 수 있는 것은 대표뿐입니다」 |
| 그룹 주보호자 | 자기 자신 | **409** 「대표는 이 방법으로 나갈 수 없습니다」 — 승계로 간다 |
| 누구든 | 그 그룹의 보호자가 **아닌** 사용자 id | **404** |
| 누구든 | 자기가 못 보는 `petId` | **404** |

⚠️ **이 경로에는 `not_group_owner`(409)가 없다.** 다른 그룹 관리 동작(초대·공통 프로필 PUT·
강아지 삭제·승계)은 그대로 409 다 — 거기는 "내 아이 화면에서 그룹 관리 버튼을 눌렀다" 라
왜 안 되는지를 주보호자 행의 아이 이름과 함께 줘야 앱이 맞는 안내를 그린다.

여기만 다른 이유 둘. ① 이 경로에서 거부당하는 사람은 **남을 내보내려 한 공동 보호자** 하나
인데, 예전에는 그가 요청한 행의 대표인지(= 기존 강아지를 연결해 참여했는지)로 409 와 403 이
갈렸다. **행 소유는 사용자도 앱도 모르는 내부 사정**이라, 화면에서 똑같은 한 동작이 두 갈래로
그려지고 앱이 두 안내를 유지해야 했다. ② 응답 코드만으로 "나는 이 행의 대표다"·"지목한 저
사람이 이 그룹의 주보호자다" 를 알아낼 수 있었다. 이제 **행 소유는 상태 코드를 가르지
않는다.** 409 는 오직 **그룹 주보호자가 자기 자신을 지목한 승계 안내** 한 줄이다.

판단 순서 (`services/pet_member.py::remove_member`):

1. `pet_repo.get_accessible` — 그 행의 구성원이 아니면 404.
2. `common_of(pet)` = 그룹 앵커 행. 이후 판단은 **전부 이 행의 대표(= 그룹 주보호자) 기준**이고,
   요청한 행의 대표(`pet.app_user_id`)는 **안 본다.**
3. 남을 내보내는 것은 그룹 주보호자만 — 아니면 403. **대표 보호(409)보다 먼저** 본다.
   순서가 반대면 공동 보호자가 주보호자 id 를 넣어 보는 것만으로 409 가 돌아와 "이 사람이
   주보호자다" 를 확인해 준다.
4. 대상이 **이 그룹의 보호자**인지 본다 — 그룹 행들의 대표 ∪ 돌보미
   (`identity_service.guardians_of`, `has_other_carers` 와 같은 정의). 아니면 404. 이 검사가
   없으면 아무 사용자 id 나 넣어도 204 가 나가 "지웠다" 와 "원래 없었다" 가 같아 보인다.
   권한을 **먼저** 보므로 주보호자가 아닌 사람은 여기 닿지도 못한다 — 남이 이 그룹 보호자인지
   떠보는 창구가 되지 않는다.
5. **한 트랜잭션에서** 그룹 **모든 행**의 그 사람 `pet_members` 행 삭제
   (`member_repo.remove_from_pets`) → `detach_user` + `prune` → 나간 사람의 `primary_pet_id`
   수선 → **commit 한 번.** 중간에 터지면 셋 다 안 일어난 것이 된다.

**그룹의 모든 행에서 지우는 것이 중요하다.** 멤버십은 `pets` **행**마다이고 연결 수락이 앵커
행 멤버십과 연결을 같이 만들어서, 한 사람이 그룹 안 여러 행의 돌보미일 수 있다. 요청받은 행
하나만 지우면 **남은 행의 멤버십으로 그룹을 계속 읽는다.**

**아무것도 지우거나 옮기지 않는다.** 나간 사람의 `pets` 행·개인 이름·개인 사진·그 행에 쌓인
기록은 그대로다. `pets.app_user_id` 도 안 바뀌고, 다른 보호자의 멤버십도 안 건드린다.

**끊기는 것은 멤버십·연결에 기대는 그룹 조회뿐이다.** 케어(`GET /app/care-events/today`)·산책
공동 조회(`GET /app/pets/{id}/walks[/{walk_id}]`)·보호자 목록이 자기 행 하나로 줄고, 나간
**뒤에** 그룹에 쌓인 기록도 안 보인다. 반대로 **작성자 기준 경로는 이번에 안 건드렸다** —
`GET /app/walks`·산책 상세·사진·일기는 "내가 만든 것" 으로 열리므로, 나가도 자기가 남긴
기록은 그대로 본다. 케어 로그 **삭제 권한**은 읽기와 같은 바닥으로 끊긴다(#574, 아래
「케어 로그 삭제 권한」) — 나간 사람은 **자기 행이 아닌 행**에 적고 간 줄을 더는 못 지우고,
그 줄은 그 행의 대표가 지운다. 기록 자체는 여전히 그대로 남는다.

부수효과 — 나간 사람의 `primary_pet_id` 가 **그룹 행 중 더는 볼 수 없는 것**을 가리키면
비운다. 대체할 아이는 남은 **구성원** 강아지 중 `list_accessible` 의 정렬(`created_at, id`)로
고르고, 남은 것이 없으면 NULL 이다. FK 가 `ON DELETE SET NULL`(`db/init/05_pets.sql:201`)이지만
**강아지 행은 안 지워지므로 안 돈다.** 자기 행처럼 **계속 볼 수 있는** 것을 가리키고 있으면
안 건드린다 — 멀쩡한 첫 화면을 흔들 이유가 없다.

**같은 요청을 다시 불러도 안전하다** (500 이 아니다). 두 번째 응답은 남은 상태에 따라 갈린다 —
연결했던 사람은 이제 자기 혼자짜리 강아지의 대표라 **409**, 연결 없이 참여했던 사람은 그 행이
더는 안 보여 **404** 다. 어느 쪽도 이미 지워진 것을 또 지우지 않는다.
...
**적은 사람은 지금도 그 행의 구성원이어야 한다** (#574). 나가거나 내보내지면
(`remove_member`) 그 행의 기록을 못 읽는데, 적은 사람이라는 것만 보면 읽지도 못하는 줄을 id 로
지울 수 있었다. 그래서 삭제 자격도 읽기와 같은 바닥(`pet_repo.member_condition`)을 따른다.
**기록은 옮기거나 지우지 않는다** — 바뀐 것은 지울 자격뿐이다. 대표 쪽은 멤버십과 무관하다(행의
대표는 그 자체로 구성원이다). 그래서:

| 부른 사람 | 지울 수 있나 |
| --- | --- |
| 그 행의 대표 (누가 적었든) | ✅ |
| 적은 사람, 지금도 그 행의 대표 ∪ 돌보미 | ✅ |
| 적은 사람, 나갔거나 내보내짐 | ❌ 404 |
| 연결한 보호자가 나간 뒤 **자기 행**에 적은 줄 | ✅ — 자기 행의 대표다 |
| 같은 행의 다른 돌보미 · 다른 행의 대표 · 남남 · 없는 id | ❌ 404 (있는지도 안 알려 준다) |

구성원 판정은 **행 기준**이다 — 논리 그룹(§6)으로 넓히지 않는다. 같은 그룹이라도 남의 행에 적힌
줄은 그 행의 대표와 그 행의 구성원인 적은 사람만 지운다.

조건은 쿼리 안에서 건다(`repositories/care_event.py` 의 `get_deletable` — `pets` 를 조인해
`pets.app_user_id = 나` 또는 (`actor_app_user_id = 나` 그리고 `member_condition(나)`) 로 묶는다).
행을 먼저 꺼내 놓고 밖에서 비교하면 부르는 쪽이 그 비교를 잊는 자리가 생긴다 —
`pet_repo.get_owned` 와 같은 이유다.
...
**보호자 내보내기(`DELETE …/members/{targetId}`)만 403 이고 `not_group_owner` 가 아니다.**
나머지 네 줄은 "내 아이 화면의 그룹 관리 버튼" 이라 행 대표에게도 이유를 줄 값어치가 있지만,
내보내기에서 거부당하는 사람은 **남을 내보내려 한 공동 보호자** 하나다. 그 사람이 요청한 행의
대표인지(= 연결해 참여했는지)로 409/403 을 가르면 화면에서 같은 한 동작이 두 갈래로 그려지고,
응답 코드가 행 소유와 "저 사람이 주보호자다" 를 알려 준다. §3 「퇴장 · 내보내기」 표를 보라.
...
### 연결 차단 — 공동 보호자가 있는 강아지 (2026-09-15 결정)

**이번 MVP 의 정책은 "현행 서비스 차단 규칙을 명문화하고 테스트로 고정" 이다.** DB 트리거·제약·
마이그레이션은 추가하지 않는다 (고도화 「서버 안전성 강화」).

- **공동 보호자가 있는 강아지는 다른 논리 강아지와 연결하지 않는다.** 수락 시 연결 대상이
  `pet_members` 에 줄을 가지고 있으면 409 `link_not_allowed` + `reason: "has_other_carers"`
  (`services/pet_member.py::_require_link_candidate`). 미리보기의 연결 후보에서도 빠진다
  (`repositories/pet.py::list_link_candidates` 조건 2).
- **막는 기준은 실제로 연결 대상으로 고른 강아지다.** 받는 사람이 주보호자라는 이유만으로, 또는
  연결하지 않을 다른 강아지에 돌보미가 있다는 이유로 초대 자체를 막지 않는다.
- **연결 없이 참여는 늘 열려 있다.** 같은 초대를 `link_to_pet_id: null` 로 수락하면 된다.
- **기존 보호자를 새 그룹에 자동 편입하지도, 자동 제거하지도 않는다.** 연결 대상의 돌보미(C)는
  연결이 막혔든 연결 없이 참여했든 그대로 그 강아지의 돌보미이고, 초대한 쪽 그룹에는 들어가지 않는다.
- **연결 전에 받는 사람이 만든 초대는 연결 뒤 무효다.** 연결되면 그 강아지의 그룹 주보호자가 초대한
  쪽으로 바뀌므로, 옛 초대는 미리보기·수락 모두 410 이다 (초대자 = 그룹 주보호자 검사). 연결 뒤 새
  보호자는 그룹 주보호자만 초대할 수 있다 (`create_invite_bundle` 의 `require_group_owner`).
- **묶음에서 한 아이라도 막히면 묶음 전체가 수락되지 않는다** (원자성, 위 검증 순서 8).
- **보호자 목록**(`GET /app/pets/{pet_id}/members`)은 이 규칙상 **유효하게 만들어진 논리 그룹 안에서**
  그룹 전체 보호자를 중복 제거해 보여 준다. 연결 전 공동 보호자가 새 그룹에 끼는 상태는 이 규칙이
  만들지 않는다 — 과거·비정상 데이터 검증과 DB 불변식은 고도화다.

앱은 이 409 를 받으면 「이 강아지는 바로 연결할 수 없어요」 안내와 연결 없이 참여 선택지를 보인다.

고정한 테스트 (`tests/test_pet_invite_accept.py` · `tests/test_pet_invite_bundle.py`):

| 경우 | 테스트 |
| --- | --- |
| 초대 생성 **뒤** 연결 대상에 공동 보호자가 생김 → 연결 409, 아무 변경 없음, C 는 A 강아지 404 | `test_초대를_받은_뒤_연결_대상에_공동_보호자가_생기면_…` |
| 같은 상황에서 연결 없이 참여 → 성공, C 자동 편입·제거 없음 | `test_같은_상황에서_연결_없이_참여하면_…` |
| 연결하지 않을 다른 강아지에 돌보미 → 고른 강아지는 연결됨 | `test_연결하지_않을_다른_강아지에_돌보미가_있어도_…` |
| 연결 전 B 의 초대를 연결 뒤 C 가 사용 → 미리보기·수락 410 | `test_연결_전에_B_가_만든_초대를_연결_뒤_C_가_쓰면_…` |
| 미리보기 연결 후보에서 돌보미 있는 강아지 제외 | `test_연결_후보에서_빠지는_넷` |
| 묶음 중 한 아이만 `has_other_carers` → 커밋 안 함 | `test_하나라도_실패하면_커밋하지_않는다` |
| HTTP 409 `reason` | `test_부적격_연결은_409_로_나간다`, `test_연결_후보_조건을_서버가_다시_본다` |
...
### 산책 기록 공동 조회 — 기존 MVP 구현 누락 보완 (2026-09-15)

MVP 결정(§7 「강아지별 산책 기록·요약·경로·게임 결과」·「수행한 보호자 정보」, §11 P0-11 「산책
공동 조회·중복 제거·수행자 표시」)은 **산책 기록 자체의 공동 조회**를 담았다. 그런데 구현은 09-12
DEV 조사 권고("`GET /app/walks` 목록은 그대로 사람 소유")를 따라 **횟수·요약·수행자 줄만** 그룹으로
넓혔고, 목록·상세·경로는 올린 사람만 봤다. 산책 횟수·집계만 공유하는 것은 요구사항을 충족하지
않는다 — **그 권고와 이 문서 「하지 않는 것 넷」의 "읽기만 연다"(소유권은 그대로 두고 읽기를 연다는 뜻은 유지)·앱 계약의 "목록에는 안 뜬다" 를 다음으로 정정한다:
조회는 그룹, 소유·수정·삭제는 작성자.**

| 경로 | 내용 |
|---|---|
| `GET /app/pets/{pet_id}/walks?cursor=&limit=` | 그 강아지가 나간 산책, 최근 순(`started_at desc, id desc`). 좌표 없음. `limit` 기본 20·최대 50, `next_cursor` |
| `GET /app/pets/{pet_id}/walks/{walk_id}` | 한 건 + 경로(`at`·`lat`·`lng` 만) |

- 항목 필드: `id`·`started_at`·`ended_at`·`duration_s`·`distance_m`·`moving_s`·`actor{app_user_id, nickname}`·
  `is_mine`·`pet_ids`. 거리·이동 시간은 `walk_analyses` **최신 세대**(게임 설정과 무관 — `activity_walk_heads`
  를 안 거친다). 봉인 전이면 null.
- **중복 제거**: 그룹의 두 행에 태그된 산책도 한 번(`walk_pets` IN 서브쿼리).
- **권한 가드**(`services/walk_group.py::readable_pet_ids`): ① 그 행의 구성원이 아니면 404, ② 연결 안 된
  강아지는 그 행만, ③ 연결된 강아지는 요청자가 **그룹 안 어느 행의 대표이거나 앵커 행의 구성원**일 때만
  `group_pet_ids_of` 로 넓힌다. 연결된 **개인 행에만** 돌보미로 있던 사람(연결 전 그 행을 돌보던 사람)은 자기
  행 산책만 본다 — 그 사람의 그룹 접근 범위는 아직 결정 전이라(#538 권한 검토) 앞서 넓히지 않는다.
- 상세는 산책이 **볼 수 있는 강아지에 태그돼 있을 때만**(IDOR). 같은 산책에 함께 태그된 그룹 밖
  강아지는 `pet_ids` 에서 뺀다.
- 나가기·내보내기는 멤버십과 연결을 한 트랜잭션에서 지우므로 그 즉시 목록·상세·경로가 404 이거나 자기
  행으로 좁혀진다. 탈퇴도 같다.
- **쓰기는 그대로다.** `/app/walks` 업로드·좌표 추가·봉인·기록 보정은 `walk.get_owned`(올린 사람), `/app/walks`
  목록도 내 산책만 준다(구 앱 호환). `pet_repo.member_condition` 은 넓히지 않았다.
- 이번 범위 밖(후속 확인): 산책 일기(`walk_entry`)·행동 핀·공간 일기의 공동 조회.

#### 산책 기록 통합 목록 — `GET /app/pet-walks` (MVP, 2026-09-15)

산책 기록 화면 「산책별」이 **내 산책과 공동 보호자 산책을 한 목록**으로 보여 주려고, 강아지 하나를 받는 위
경로와 별도로 **볼 수 있는 강아지 전부**를 범위로 읽는 목록을 둔다. 위 두 경로의 계약은 바꾸지 않았다.

| 조건(쿼리) | 뜻 |
|---|---|
| `pet_id` (반복) | 요청자 화면의 강아지 id(`/app/pets` 카드). 없으면 전부. **볼 수 없는 id 는 빠질 뿐 범위를 넓히지 않는다**(전부 볼 수 없으면 빈 결과) |
| `actor_id` (반복) | 올린 사람. 없으면 전부 |
| `date_from`·`date_through`·`tz` | 요청자 시간대(IANA, 기본 `UTC`)의 날짜 경계 |
| `month` (반복) | 요청자 시간대의 시작 월 — 앱의 계절 조건 |
| `weather_code` (반복)·`weather_missing` | 출발 날씨 WMO 코드, 날씨 없는 산책 포함 여부. 분류(맑음·비…)는 앱이 코드로 풀어 보낸다 |
| `exclude_mine` | 내가 올린 산책을 뺀다 — 앱은 내 산책을 기기 기록(Room)으로 이미 갖고 있다 |
| `cursor`·`limit` | 위 목록과 같은 최근 순 커서, 기본 20·최대 50 |

- 응답: `walks[]`·`totals{count, distance_m, duration_s}`·`carers[]`·`next_cursor`.
  - `walks[]` 는 위 항목 필드에 `weather_code`·`is_day`·`temperature_c`·`route_preview` 를 더한다. `route_preview` 는
    `chain_index` 마다 끊은 `[{lat, lng}]` 목록(한 산책 최대 약 48점)으로 **카드 썸네일 전용**이다 — 카드마다
    상세를 부르지 않게 하는 자리다. 경로 원본은 여전히 상세(`/app/pets/{pet_id}/walks/{walk_id}`)에만 있다.
  - 여기의 `pet_ids` 는 **요청자 화면의 강아지 id** 로 바꿔 준다(연결된 그룹에서 남의 행에 태그된 산책도 내 카드
    id). 위 목록의 `pet_ids` 는 행 id 그대로다.
  - `totals` 는 **페이지가 아니라 조건 전체**다. 거리는 산책마다 최신 계산 세대 하나(계산 전은 0), 시간은
    `ended_at - started_at` 합.
  - `carers` 는 조건과 무관하게 볼 수 있는 강아지 전부의 **지금** 구성원(대표 ∪ 돌보미)을 사람 단위로 중복 제거한
    보호자 후보다. `is_me` 가 먼저, `pet_ids` 는 그 사람과 함께 돌보는 내 카드 id.
- 범위는 카드마다 `readable_pet_ids` 를 그대로 부른다 — 권한 규칙을 다시 쓰지 않는다. 나가기·내보내기·탈퇴 뒤에는
  그 즉시 목록·합계·후보에서 빠진다.
- 앱은 목록·기간·계절·날씨·강아지·보호자 조건만 서버로 보낸다. **제목·메모 검색과 행동 조건은 공동 보호자
  산책에 적용할 수 없어**(서버에 제목·메모가 없다) 그 조건이 켜진 동안 앱이 공동 보호자 산책을 빼고 알린다.
- 「모아보기」 지도는 이번 범위가 아니다 — 계속 내 산책 경로만 그린다.

### 게임·점령 공유 — 이번 결정으로 후순위 보류

게임·점령 결과 공유도 **원래 의도(MVP 결정 §7 「게임 결과」)에 포함돼 있었다.** 점수·순위·보상에 미치는 영향과
작업 범위를 고려해 **이번 단계에서 후순위로 보류**한다 — 처음부터 제외된 요구사항이 아니다.

- 공동 조회 응답·화면에 게임·점령 결과를 싣지 않는다(응답 키 집합을 테스트로 고정).
- 개인 게임 기능·점령지·점수·순위·보상 계산과 소유권은 그대로다. 산책 조회 권한을 게임·점령 API 로 넓히지 않는다.

추후 고도화 항목(서로 다른 결정이다):

1. **산책별 게임·점령 결과의 읽기 전용 공유**
2. **점령지·점수·보상 등의 공동 소유 및 집계**
3. **그룹 가입·탈퇴가 게임 권한과 순위에 미치는 영향**
...
(`remove_member` → `detach_user` → `prune`, commit 은 끝에 한 번). 멤버십만 지우고 연결이
남으면 나간 사람의 pet 행이 그룹에 그대로 있어 **공동 조회로 남의 집 기록을 계속 읽는다.**
멤버십은 그룹의 **모든 행**에서 지운다 — 갈래와 응답은 §3 「퇴장 · 내보내기」 표에 있다.
...
#### 보호자 그룹 병합 (2026-09-15 결정 — 이번 MVP 미구현)

지금은 공동 보호자가 있는 강아지를 **연결하지 않는다** (위 「연결 차단」). 같은 실제 강아지로
판단되는 두 강아지에 각각 공동 보호자가 있다면, 고도화에서는 **자동으로 합치지 않고 보호자 그룹
병합 절차**를 제공한다.

예: A 의 강아지 `a`, B 의 강아지 `b`, C 는 `b` 의 공동 보호자. A 가 B 를 초대하고 B 가 `b` 를 `a` 에
연결하려는 상황.

검토할 흐름:

1. 연결 전에 양쪽 강아지의 기존 보호자와 공유되는 정보 범위를 안내
2. 영향을 받는 주보호자와 공동 보호자에게 병합 동의 요청
3. 필요한 사람의 동의가 모두 완료된 경우에만 두 강아지를 논리적으로 연결
4. 양쪽 보호자 목록을 중복 제거해 하나의 공동 돌봄 그룹으로 병합
5. 병합 후 전체 보호자가 같은 강아지의 허용된 케어·산책 기록과 보호자 목록을 조회
6. 그룹 전체 보호자 최대 5명 제한 검사
7. 한 명이라도 거절하거나 동의가 만료되면 연결과 권한 변경을 전부 취소
8. 그룹 주보호자를 누구로 유지할지 명확히 결정하고 병합 전에 표시
9. 병합·거절·취소 이력과 감사 로그 보존
10. 추후 연결 해제 시 각 원본 강아지와 보호자 관계를 안전하게 복원하는 정책 마련

#### 서버 안전성 강화 (2026-09-15 결정 — 이번 MVP 미구현)

- 서비스 코드뿐 아니라 DB 에서도 잘못된 보호자·논리 연결 조합을 방지하는 불변식
- 동시 요청에서도 보호자 상한과 연결 상태가 깨지지 않도록 잠금 처리
- 과거 데이터 및 비정상 데이터 검증
- 병합 요청의 원자성·멱등성·권한 검증
- 관계자별 동의 상태와 만료·취소 처리
