...
def edit(operation="exclude", kind="ordinal", text="첫 번째", quote="빼고"):
    return {
        "operation": operation,
...
async def chat(service, previous, query="첫 번째 빼고 더 보여줘", **intent):
    service.planner.next = Interpretation(goal="show", **intent)
    return await service.prepare(
        None, PrepareRequest(mode="chat", query=query, previous=previous.state)
    )
...
async def test_exclude_precedes_limit_persists_on_manual_change_and_restores():
    ...
    first = await service.prepare(None, manual())
    excluded = await chat(
        service, first, query="전부 빼고 보여줘", place_edit=edit(kind="all", text="전부")
    )
    ...
    restored = await chat(
        service, moved, query="첫 번째 다시 넣어줘", place_edit=edit("restore", quote="넣어줘")
    )
...
async def test_exclusion_resolves_actual_display_order_and_restart_clears_only_exploration():
    service, _ = make_service()
    first = await service.prepare(None, manual())
    service.planner.next = Interpretation(goal="show", place_edit=edit(), browse="next")
    ...
    restarted = await chat(service, edited, query="제외도 풀고 처음부터 다시", browse="restart")
...
async def test_failure_does_not_consume_page_or_commit_exclusion_then_retry_succeeds():
    ...
    first = await service.prepare(None, manual())
    searcher.error = True
    failed = await chat(service, first, browse="next", place_edit=edit())
    ...
    retried = await chat(service, failed, browse="next", place_edit=edit())
...
async def test_expired_snapshot_next_still_omits_presented_and_excluded():
    ...
    first = await service.prepare(None, manual())
    excluded = await chat(service, first, place_edit=edit())
...
async def test_invalid_target_and_bounded_record_do_not_report_exhaustion():
    service, searcher = make_service()
    first = await service.prepare(None, manual())
    invalid = await chat(service, first, query="21번 빼고", place_edit=edit(text="21번"))
...
async def test_exploration_with_unsupported_requirement_is_not_partially_applied(operation):
    service, _ = make_service()
    first = await service.prepare(None, manual())
    result = await chat(service, first, place_edit=edit(operation), unsupported=["quiet"])
