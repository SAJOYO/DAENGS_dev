...
- 원본 GPS 확인에 사용하는 `walk_chunk`·`walk_recording`은 이번 패키징 밖에 남는다. 해당 원본 기능은 8단계 대상이며 핀 검증 정책을 바꾸지 않는다.
...
8단계 원본/업로드/봉인, 9단계 측정/공간 조회, 10단계 남은 과거 호환과 통합 검증이 남아 있다. 0~5단계에서 해결한 결합이나 이번 6~7단계의 패키징 성과를 산책 전체 완료로 확대하지 않는다.
