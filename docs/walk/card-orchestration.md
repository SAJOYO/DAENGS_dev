...
제목 배치를 카드별 캐시로 풀기 **전에 원래 배치의 요청 지문을 현재 모델·해당 전략 프롬프트와 대조한다**(#509). 제목 프롬프트나 모델이 달라졌으면 옛 제목을 현재 키로 다시 등록하지 않는다. 공간 프롬프트만 달라지고 채택된 카드 본문이 같다면 제목은 계속 재사용할 수 있다. 이미 발행한 보드는 정책 변경만으로 다시 쓰지 않으며, 원문 수정으로 새 생성이 필요할 때 이 캐시 검사가 적용된다. 2026-09-14의 [회귀 검사](../../backend/tests/walk/diary/test_diary_title_cache.py)는 배치/개별 제목 캐시, 정책별 재사용, 실패 시 기본 제목, HTTP 공개본 보존과 새 원문 재생성을 확인한다. 모델과 외부 수집은 대역이다.
