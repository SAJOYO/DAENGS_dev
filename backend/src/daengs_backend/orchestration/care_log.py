"""케어 기록 쓰기의 **결정론 부분** — 무엇을 기록할지와, 사용자가 승낙했는지 (#331 후속, D-075).
...
_NOT_A_STATEMENT = re.compile(
    ...
    # ── 상태 보고. 기록이 아니라 **관찰**이다.
    #
    # `"밥은 잘 먹고 산책도 평소처럼 잘 했어요"` 가 이 줄을 만들었습니다 — 골드 세트를 훑는
    # 테스트가 잡았고, 그것은 되묻기에 **답하는** 발화입니다(`evals/ask_loop`). 기록 확인으로
    # 가로채면 비서가 방금 물어본 것에 아무도 답하지 않습니다. `잘 먹`·`평소`·`여느`·`똑같`
    # 은 "얼마나/어떻게" 를 말하는 표현이라 한 건을 적겠다는 말이 아닙니다.
    #
    # `산책` 이 여기 있는 것은 종류 목록에 `walk` 가 없는 것과 같은 결정입니다 — 산책이 같이
    # 언급된 발화는 한 항목을 적겠다는 말이 아니라 **하루를 보고하는** 말입니다.
    r"잘 ?먹|평소|여느|똑같|그대로|산책|"
    # ── 남의 말 옮기기. `"…라고 했다"` 는 "내가 했다" 가 아니다.
    #
    # 이것도 같은 테스트가 잡았습니다 — 걸린 것이 전부 평가 판정문
    # (`"사료 포장지의 권장량을 참고하라고 했다"` 류)이었습니다. 사용자 발화는 아니지만,
    # 인용을 기록으로 읽는 것은 어휘가 아니라 **뜻**의 문제라 여기서 막습니다.
    r"라고 ?했|다고 ?했|하라고|말했|답했|권했"
)
