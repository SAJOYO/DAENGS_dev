...
> **과거 #508 구현 기록.** 아래의 ‘이동만으로 활동 작성’, 메모·사진 카드의 활동 생성,
> 원문을 포함한 제목 입력은 사용자 규칙과 달라 #527에서 폐기했다.
> 신규 작성의 기준은 [행동 핀 생성 경계와 독립 메모](diary-action-boundary.md)다.
> 이동 분석·속도 정책과 과거 공개본은 유지한다. 아래 실험은 당시 구현의 기록이며 현재 수용 기준이 아니다.

2026-09-14, DEV #508. 기존 카드 그래프의 실제 작성·저장·재조회 경로에 연결했다.
...
직선·회전·되짚기·국소 체류는 [기존 경로 판별기](route-patterns.md)를 재사용한다.
...
회전은 기존 꼭짓점 시간·거리 제한도 통과해야 한다. 전후 관측 범위와 회전 시점을 구별하여
꼭짓점이 포함된 단계에만 연결하고, 작성 입력의 `at_s`로 실제 전환 시각도 보낸다.
...
작성 입력은 `diary-prose-input-v3`의 허용 필드로 새로 만들고, 짧은 별칭으로 연결한다.
...
```json
{
  "movement": {
    "phases": [{
      "from_s": -20,
      "to_s": 0,
      "path": [{"id": "m1", "meaning": "방금 지나온 구간을 되짚는 이동"}],
      "pace": [{"id": "m2", "meaning": "이번 산책의 기준 속도보다 상대적으로 느린 이동"}]
    }]
  },
  "recorded_action": {"id": "a1", "actor": "보리", "action": "냄새 맡기", "at_s": 0}
}
```

예시의 초는 기록 시각 기준이다. 원 좌표·측정 배열·m/s·임계값·해시·탈락 사유는 보내지 않는다.
...
응답은 `text/evidence_ids`다. 별칭을 **장면 적용 구간별 근거**로 복원하고,
