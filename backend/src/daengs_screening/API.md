...
정답 대비 **크기 0.71~1.67배** 가 허용, **0.83~1.43배** 가 권장입니다 (STEP 16).
1.5배로 그리면 허용 안이지만 권장 밖입니다. 서버가 추론 **전에** 검사해서 밴드 밖이면
...
* **2단계 백본이 확정됐습니다** — `convnextv2_base`, holdout macro-F1 0.5990
  [0.5914, 0.6058] (STEP 16). 계약은 안 바뀌었고 숫자만 바뀌었습니다.
* ⚠️ **촬영 가이드 밴드가 STEP 10 → STEP 16 으로 바뀌었습니다** (위 "네모를 얼마나").
  예전 값(허용 0.59~1.43 / 권장 0.71~1.18)이 앱에 박혀 있으면 같이 고쳐야 합니다.
  **작게 그리는 쪽은 너그러워지고 크게 그리는 쪽이 빡빡해졌습니다** — 2.0x 에서
  macro-F1 이 0.449 입니다.
