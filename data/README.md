...
│   └── eval/         #   6단계 3파전 문항별 top-k 덤프 (jsonl) — 기계 산출물이라 무시 (RAG-024 ④)
├── eval/             # 손으로 만드는 평가 자산 자리. git 관리 대상이나 **지금은 비어 있다** —
│                   #   골든셋은 RAG-022 ⑥으로 backend/rag/stages/goldenset.yaml 로 갔다
└── reference/        # 정적 참조 데이터 (에어코리아 측정소 목록, 기상청 격자 변환표 등)
```

### git 추적 범위 (RAG-008 → RAG-017)
...
| `raw/**/*.meta.json` | ❌ 무시 (RAG-017 에서 변경) |
| `manifests/crawl_log.jsonl` | ❌ 무시 |
| `processed/**` | ❌ 무시 |
| `eval/`, `reference/` | ✅ 커밋 — 손으로 만드는 것이라 성격이 다르다. **기계 산출물은 여기 오지 않는다** (RAG-024 ④) |
...
3. **출처 메타 필수** — 원본 1개당 같은 이름의 `.meta.json` 1개.
   ...
   - `sha256`은 **콘텐츠 지문**이다. RAG-001 원칙2의 변경 감지용으로, 이전 값과 같으면 재파싱·재임베딩을 건너뛴다.
     - 단, sha256이 같아도 **`raw_file`이 가리키는 원본이 디스크에 없으면 다시 받는다** (RAG-010).
   ...
   - `cites` — 본문이 인라인 인용한 조항. RAG-004 `section` 형식(`제16조제2항제1호`)으로 정규화.
...
4. **크롤 로그** — `manifests/crawl_log.jsonl`. 한 줄에 JSON 객체 하나(JSON Lines), append-only.
   ...
   없어 복구, RAG-010) / `forced`(`--force`) / `same`(스킵). `changed:true` 만으로는 내용이 바뀐 것인지
   ...
   - `run_id` 단위 요약은 Postgres `crawl_runs` 테이블에 별도 기록한다 (RAG-001 원칙3, 관리자페이지용).
...
> **`care`·`emergency`는 제외했다 (RAG-008).** 진단 성격이 강해 별도 에이전트로 분리한다.
...
## PDF 파서 — **`PyMuPDF` 로 확정** (2026-08-28, RAG-032)

실물 약관 PDF 로 재고 정했다. 표본은 코레일 여객운송약관(28p) · 광역철도(44p) ·
삼성화재 펫보험 약관(202p). **아래 표는 실측값이고, 이전 판의 사전 평가와 반대다.**

| | 형태 | 유효표(삼성 60p) | 조항 줄머리 | 속도(20p) | 라이선스 |
|---|---|---|---|---|---|
| `pypdf` | 순수 Python | 표 기능 없음 | 60% | 1.4s | BSD |
| `pdfplumber` | 순수 Python | **6 / 101** | 52% | 3.4s | MIT |
| **`PyMuPDF`** | C 바인딩(MuPDF) | **24 / 28** | 60% | 1.4s | ⚠ AGPL-3.0 |

유효표 = 행 2개 이상 · 열 2개 이상 · 빈 셀이 절반 미만. 청커가 `헤더: 값` 으로 짝지으려면
(RAG-004 ③(나)) 이 조건이 필요하다.

**개수보다 내용이 갈랐다.** 보장한도·면책기간 페이지에서 `PyMuPDF` 는
`구분 | 면책기간 적용 담보 | 면책기간` 을 값과 함께 복원하는데, `pdfplumber` 는 `면책기간`·`보험금`·
`미지급` 을 **1칸짜리 표로 쪼개 값을 잃는다.** 이 문서가 경고했던 "셀 값이 한 줄로 뭉개지는" 실패다.

**AGPL 은 오프라인 격리로 해소한다.** PDF 파싱은 `rag parse` 배치이고 `app`·`daengs_backend` 는
parse 단계를 import 하지 않는다. `[dependency-groups]` 의 `pdf` 그룹으로 두어 backend 컨테이너에
넣지 않는다 (`ml` 그룹이 torch 를 격리한 것과 같다, D-021). **PDF 파싱이 런타임으로 들어가면
이 결정을 다시 본다.**

- **스캔 PDF 는 표본 4개에서 0건.** 텍스트 레이어가 다 있다 — OCR 폴백은 지금 필요 없다 (D-006 첫 실측).
  보험사 9사 전수에서 다시 잰다.
- HWP 는 `pyhwp`(hwp5) 설치가 까다롭고 실패율이 높다. HWPX 는 표준 라이브러리(`zipfile`+`xml.etree`)로 처리된다.
