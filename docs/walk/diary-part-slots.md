...
| 서술 | `gemini-3.1-flash-lite` 1회, writer 상한 15초. 서비스는 요청의 기존 공개 마감(최대 10초)이 먼저 적용됨. 장면·슬롯 선택은 코드, Gemini는 배경 문장만 작성 |
...
요청 시작 시각 기준의 마감이 우선한다(현재 최대 10초). 시간 초과·취소 뒤 늦은 응답은
