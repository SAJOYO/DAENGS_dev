...
**기존 보드 제외는 조회·편집 계약을 단계적으로 연결하기 위한 정책이다.** #404 당시에는 배경이
바뀌면 기존 결과가 stale로 숨겨졌다. [#406](https://github.com/SAJOYO/DAENGS_dev/pull/406)은
원본이 동일한 완성 일기를 유지하지만, 이전 저장 형식의 이미 낡은 결과와 생성 중·실패 결과까지
모두 보존하는 것은 아니다. 보강만으로 사람이 보던 일기가 바뀌지 않도록
