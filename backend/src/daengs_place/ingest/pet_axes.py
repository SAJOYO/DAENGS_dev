...
문턱값(`app/geo/pet.py` 의 kg 등급 경계)이 바뀌면 이 단계만 다시 돌린다 — 원천을 다시 받지 않는다.
    ...
    python -m daengs_place.ingest pet-axes            # 축이 비어 있는 행만
    python -m daengs_place.ingest pet-axes --all      # 전부 다시 파생 (문턱값 변경 후)
"""
...
from daengs_place.geo.pet import derive_axes
