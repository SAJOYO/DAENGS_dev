...
from daengs_backend.services.walk_background.catalogs.park import nearby_parks, read_catalog
...
async def collect_public(tag, point, pin=None, *, transport=None):
    ...
    if tag in {"space.commerce", "space.river"}:
        from daengs_backend.services.walk_background.providers.area import collect_area
    ...
    try:
        ...
        if not address:
            from daengs_backend.services.walk_background.catalogs.regions import can_prepare
