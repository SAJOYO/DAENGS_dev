"""골드 80개를 두 구현에 각각 먹이고 같은 자로 잰다 (#252, D-055 ⑥).
    ...
    uv run python -m daengs_evals.orchestrator_comparison.runner --limit 3   # 연기 시험
    uv run python -m daengs_evals.orchestrator_comparison.runner             # 전체
...
`daengs_evals.router_benchmark.evaluate.evaluate_benchmark` 를 **그대로** 쓴다 (카드 ①).
...
- 채점기: `daengs_evals/router_benchmark/evaluate.py` (v1~v8 과 **같은 자**)
