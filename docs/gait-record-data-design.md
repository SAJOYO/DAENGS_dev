...
관련 코드: `gait-analysis/`(D-028), `backend/`. 관련 결정: D-011(MVC2·SQLAlchemy),
D-021(daengs_life 상주 프로세스), D-022·D-024(스크리닝 분리), D-028(보행 분석 분리).
...
서비스이고(D-028), 일반 서비스 DB는 아직 스키마가 없습니다. 그래서:
...
`gait-analysis`는 이미 독립 서비스입니다(D-028). `backend/`와 다른 레이아웃, 자체
...
이유는 D-022·D-024·D-028과 같습니다 — **torch 무게, 프로세스가 죽는 범위, 영상은
...
그동안 backend의 워커 하나가 그 요청을 물고 있게 되고, 로그인·`/ask` 같은 이미
...
항상 gait-analysis가 떠 있어야 가능하다는 것인데, 이건 이미 D-024/D-028이 받아들인
