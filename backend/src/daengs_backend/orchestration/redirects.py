...
#:
#: ⚠️ **"수의사 등 전문가" 다** (2026-09-17 사용자 확정). 앱 비교 카드가 같은 화면에서 이미
#:    "수의사 등 전문가와 상담하는 것을 권장해요" 라고 말하고 피부도 "수의사 진료" 를 쓰는데,
#:    서버 보행만 "전문가" 여서 **한 화면에서 두 낱말이 갈렸다.** 그 불일치를 없앤 것이다.
#:    ⚠️ 그래도 **모델은 여전히 "수의사"를 못 쓴다** — `_VET_TERMS` 가 막는다. 이 낱말은
#:    코드가 쓴 조건부 문장에만 있고, 행동 집합에 진료 권유가 생긴 것이 아니다(D-058).
#:    이 줄은 권유가 아니라 **선택지를 알려 주는 말**이라는 성격도 그대로다.
#: ⚠️ 정도(심하다 · 악화 · 질환 의심)를 말하지 않는다. 영상만으로는 원인을 알 수 없다는
#:    사실이 이 문장의 앞 절이고, 그것이 이 줄이 과장으로 읽히지 않게 하는 장치다.
GAIT_EXPERT_ADVISORY = (
    "영상만으로는 원인을 알 수 없어요. 이런 변화가 다음에도 반복되면 수의사 등 전문가의 "
    "의견을 받아 보는 것도 좋아요."
)

#: 보호자가 **이미 받은 진단**을 말한 대화에서 덧붙는 한 줄. 붙일지 말지는 모델이
#: `GaitGuidance.owner_condition` 으로 알리고 **코드가 대조한 뒤** 이 문장을 쓴다 — 모델이
#: 직접 쓰지 않는 이유는 `GAIT_EXPERT_ADVISORY` 와 같다 (제품 문장은 코드가 쓴다, D-080).
#:
#: ⚠️ **뒷문장을 떼지 마세요.** 앞문장만 남으면 이 보행 비교가 그 병의 근거로 읽히고, 그것이
#:    D-058 이 막으려던 바로 그것입니다. 앞문장은 보호자의 걱정을 받아 주는 말이고, 뒷문장이
#:    그 걱정과 이 비교 사이에 선을 긋습니다. 둘이 한 쌍이라야 이 줄이 안전합니다.
#: ⚠️ `{condition}` 에는 **보호자가 쓴 낱말이 그대로** 들어갑니다 — 어댑터가 보호자 쪽
#:    텍스트와 대조해 통과시킨 것만 옵니다. 여기서 말을 다듬지 마세요.
#: ⚠️ **칸이 둘입니다.** `{particle}` 은 받침에 따라 갈리는 주격 조사(이/가)이고
#:    `_subject_particle` 이 채웁니다 — "탈구**가**" 와 "관절염**이**" 가 갈려야 합니다.
#:    문구를 바꿀 때 **두 칸을 붙여서** 두세요. 조사를 문장에 직접 박으면 한쪽이 틀립니다.
#: ⚠️ `GAIT_EXPERT_ADVISORY` 와 **같이 나가지 않습니다** — 둘 다 전문가를 말하므로 어댑터가
#:    하나만 고릅니다.
#: ⚠️ **"수의사 등 전문가"** 는 앱 비교 카드와 맞춘 낱말입니다 (2026-09-17 사용자 확정) —
#:    `GAIT_EXPERT_ADVISORY` 와 같은 이유이고, 바꾸실 때 **둘을 같이** 바꾸세요.
#:
#: 문구는 사용자가 확정합니다 (2026-09-17 현재 초안). **바꿀 때 이 상수만 고치면 됩니다.**
GAIT_OWNER_CONDITION_ECHO = (
    "말씀하신 {condition}{particle} 걱정되신다면 수의사 등 전문가와 상담해 보세요. "
    "다만 이 비교만으로 그 판단을 할 수는 없어요."
)
