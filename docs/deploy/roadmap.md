...
5. **DB 는 dev/prod 로 갈라진다.** GCP 2대(pgvector·place-db, 덤프 복원)가 운영 정본,
   ...
   > 를 뺐다 (`docs/life/roadmap.md` §4 의 3번 · §6).
   >
...
쓰지 않기로 하면서 아래 1번이 **지금 열린 카드**가 됐다 — Life 로드맵 §4 의 3번이 그것을 가리킨다.
2~4번의 순서는 안 바꿨다.

1. **크롤러·코퍼스·적재 이전** — 🟢 **열린 카드다** (Life §4 의 3번). `DAENGS_CORPUS_DIR` 정본 컷오버(+로컬 워커 정지 **같은 날**),
