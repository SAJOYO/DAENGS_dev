...
| 회원이 탈퇴 · 정지를 요청했다 | 🔵 **찾는 것까지는 콘솔** (#211). 상태를 바꾸려면 아직 SSH 터널(15432) → psql | 상태 변경과 원문 복호화 | A2b |
...
| A2b | **원문 복호화와 상태 변경** — `pii:read` 원문 보기 + `admin.app_user.pii_revealed` 감사 · `PATCH status` + 세션 끊기 | 감사에 남길 가치가 있는 것은 "가려서 봤다"가 아니라 "원문을 열어 봤다" — 그 선에서 A2 를 갈랐다 | A4 · **#211** · 복호화는 `core/crypto.py` 만 (D-012) | S | ⬜ #212 |
...
A4 감사 로그 ✅ → A3 계정 관리 → A2a 회원 조회 ✅ → A2b 복호화·상태 변경
                                                  (A 트랙. A4 가 앞인 이유는 아래)
※ A2a 는 A3 을 안 기다렸다 — 나가는 값이 전부 마스킹이라 `READ` 로 열리고, `pii:read` 가
   실제로 갈려야 하는 것은 A2b 다.
