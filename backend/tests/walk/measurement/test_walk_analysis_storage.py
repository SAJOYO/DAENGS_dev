...
def test_sqlalchemy_metadata_keeps_state_identity_and_cascade_contracts() -> None:
    ...
    assert [column.name for column in WalkCapsule.__table__.primary_key.columns] == ["analysis_id"]
...
def test_models_compile_to_postgresql_jsonb_contract() -> None:
    analysis_sql = str(CreateTable(WalkAnalysis.__table__).compile(dialect=postgresql.dialect()))
    sheet_sql = str(
        CreateTable(WalkCellophaneSheet.__table__).compile(dialect=postgresql.dialect())
    )
    capsule_sql = str(CreateTable(WalkCapsule.__table__).compile(dialect=postgresql.dialect()))
...
def test_init_and_idempotent_migration_define_the_same_storage_boundary() -> None:
    ...
    capsule_migration = (REPO / "db/migrations/2026-09-03_walk_capsules.sql").read_text(
        encoding="utf-8"
    )
    capsule_required = {
    ...
    assert all(
        token in init_sql for token in capsule_required - {"ON CONFLICT (analysis_id) DO NOTHING"}
    )
