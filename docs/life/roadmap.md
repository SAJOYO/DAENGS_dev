...
**lap18 15·15** (A3a) → lap19 17·15 → lap20 18·16 → **lap21 18·15**(33문항).
...
| D5d | **골든셋 라벨 보강** — Q3·B1 에 `별표 4-2-아` 를 OR 대안으로 | RAG-057 ⑦ — 가장 좋은 근거가 애초에 라벨에 없었다. RAG-029 가 예고한 자리 | ⬜ **사람 확인 뒤** (지표가 움직인다) |
...
| D8 | **부풀림 쪽 확인** — `knia-disclosure` 2문항이 `cited` 2 · `grounded` 0 이다. 답이 맞는데 라벨이 좁은 것인지(RAG-059 · RAG-056 의 모양) 정말 틀린 것인지 | RAG-060 ⑦ — 이 카드가 보이게만 하고 안 봤다 | ⬜ **D5d 와 같은 층** (라벨을 보는 자리라 같이 보는 것이 싸다) |
...
D5c ✅ #218 (지표는 안 움직였다 — RAG-058 ④).  D5d 는 사람 확인이 먼저다.
```
