...
진행 중인 장면 근거 연결: 이미 고른 보드의 중심 참조·시각·위치에 수집 결과를 묶고,
경로 보충 지점도 기존 공간·환경 판정과 적재를 사용하도록 연결한다. 작성 입력에는 장면의
중심과 시간 관계를 보존한다. 실제 산책의 보존 입력으로 재현·비교하며 발행본을 덮어쓰지 않는다.
구현·검증 결과는 이 문서의 후속 절에서 갱신한다.
