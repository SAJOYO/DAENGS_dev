...
| 그룹에 자기 행은 있지만 주보호자가 아닌 사람 | 남 | **409** `not_group_owner` (+ 그룹 주보호자 행의 아이 이름) |
| 요청한 행의 대표가 아닌 돌보미 (앵커 행 id 로 부른 연결 보호자 포함) | 남 | **403** 「다른 보호자를 내보낼 수 있는 것은 대표뿐입니다」 |
| 누구든 | 그 그룹의 보호자가 **아닌** 사용자 id | **404** |
| 누구든 | 자기가 못 보는 `petId` | **404** |

409 두 줄이 갈리는 이유 — 자기 행을 가진 사람에게는 감출 것이 없으므로(자기 화면에서 부른
것이다) 왜 안 되는지를 아이 이름과 함께 준다. 404 로 답하면 앱이 "강아지를 찾을 수 없습니다"
라고 거짓말을 그린다. 그 행의 대표조차 아닌 돌보미에게는 이유를 안 주고 403 만 준다.

판단 순서 (`services/pet_member.py::remove_member`):

1. `pet_repo.get_accessible` — 그 행의 구성원이 아니면 404.
2. `common_of(pet)` = 그룹 앵커 행. **대표 보호는 공통 행의 대표(= 그룹 주보호자)에게만** 건다.
3. 남을 내보내는 것은 그룹 주보호자만 (위 표의 409 · 403).
