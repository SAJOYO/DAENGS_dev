...
산책 전의 `GET /walk`와 산책 후의 `POST /weather/at`을 분리합니다. 전자는 현재와 미래를
조립해 산책 적합도를 판단하는 제품 읽기이고, 후자는 좌표와 과거 시각에 대응하는 환경 사실을
판단어 없이 반환하는 Life capability입니다. 과거 조회가 현재 산책 적합도 전체를 재실행하면
예보·대기질·특보까지 불필요하게 호출하고, 현재값을 과거 사실로 오인할 여지가 생깁니다.
...
중복 전송만 줄이는 캐시이지 영구 기록은 아닙니다.

Life는 환경 사실을 조회할 뿐 Walk, Capsule, 일기, pet을 소유하지 않습니다. 후속 Walk 조립은
산책의 대표 시각과 좌표를 선택해 이 capability를 호출하고 응답값을 Capsule에 동결합니다. 이번
단계에는 Walk DB 저장, finalize 연결, 앱 호출, Place 주변 사실, Journey 계획 경로를 포함하지
않습니다. 이 경계는 D-046의 소유권을 그대로 유지합니다.
