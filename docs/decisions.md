...
| [D-052](#d-052) | 공용 파일 저장소는 GCS 버킷 하나 — 접두사로 도메인을 가르고, 보관은 탈퇴 시 파기 하나다 | 2026-09-04 |
...
## D-052
### 공용 파일 저장소는 GCS 버킷 하나 — 접두사로 도메인을 가르고, 보관은 탈퇴 시 파기 하나다

#78 이 물은 두 가지에 대한 답입니다 — **A. 어디에 둘 것인가**, **B. 보관 정책**.
사람 결정(2026-09-04)이고, 그 카드는 이 기록으로 닫힙니다.

코드는 이미 이 답을 기다리는 모양으로 서 있었습니다. `core/storage.py` 의
`GcsStorage` 는 V4 Signed URL·조건부 읽기·`redact` 까지 **완성돼 있고**,
`google-cloud-storage` 는 기본 런타임 의존성입니다. `settings.gait_storage` 의 기본값이
`none` 이라 `NotConfiguredStorage` 가 모든 호출을 503 으로 돌려보내고 있었을 뿐입니다.
그래서 이 결정에는 **새 코드가 없습니다** — 버킷을 만들고 `.env` 네 줄을 채우는 일입니다.

#### A. 버킷은 **하나**다

provider 는 D-043 이 GCS 로 확정했습니다(2026-09-02). 이 결정이 더하는 것은 **개수**입니다.

**도메인마다 파지 않습니다.** 지금 저장소를 쓸 자리가 보행 영상·점령지 사진에 더해
프로필 사진·피부 사진·도감 카드까지 다섯인데, 버킷을 다섯 개 파면 권한·수명주기·감사가
다섯 벌이 됩니다. 그런데 `settings` 는 `gait_gcs_bucket` **하나**를 들고 있어서,
도메인별 버킷은 곧 설정과 팩토리를 도메인 수만큼 가르는 일이 됩니다. 얻는 것이 없습니다 —
버킷을 나눠서 생기는 격리는 접두사와 IAM 조건으로도 같은 만큼 얻습니다.
...
이미 `build_object_key` · `build_territory_photo_key` 가 그 모양입니다. 새 도메인은
**key builder 를 하나 더 두는 것으로 끝납니다.** 키를 backend 만 만든다는 원칙 ⑥ 은
그대로입니다 — 앱이 키를 정하면 남의 경로를 덮어쓰거나 훔쳐볼 수 있습니다.

##### 버킷 설정

| 항목 | 값 | 왜 |
| --- | --- | --- |
| 이름 | `daengs-media-prod` | 전역 유일해야 하므로 겹치면 뒤에 프로젝트 id 를 붙입니다 |
| 리전 | **`asia-northeast3`** (서울) | 보호자 계정에 묶인 사진은 개인정보라, 해외 리전이면 **국외이전 동의**가 따로 필요해집니다 |
| 클래스 | Standard | |
| 접근 제어 | Uniform bucket-level access | 객체마다 ACL 을 두지 않습니다 |
| 공개 접근 방지 | **적용(enforced)** | D-043 원칙 ⑦ — 버킷은 public 으로 열지 않습니다 |
| 객체 버전 관리 | **끈다** | ⚠️ 켜면 `redact()` 가 무의미해집니다. 점령지 사진은 판정이 끝나면 0바이트 tombstone 으로 덮어 원본을 지우는데, **버전 관리가 켜져 있으면 옛 버전에 원본 사진이 그대로 남습니다** |
| 소프트 삭제 보존 | **0 (끈다)** | ⚠️ GCS 는 지운 객체를 얼마간 되살릴 수 있게 붙들어 둡니다. B 의 "탈퇴 시 지체 없이 파기"와 정면으로 어긋납니다 |
| 수명주기 규칙 | 없음 | B 가 기간을 두지 않기 때문입니다 |
| CORS | 설정 안 함 | 올리는 쪽이 네이티브 앱이라 브라우저 CORS 가 안 걸립니다. 웹이 올릴 일이 생기면 그때 |

##### 권한 — Signed URL 이 여기서 깨집니다

VM 에 붙은 서비스 계정에 **버킷 단위로** `roles/storage.objectAdmin` 을 줍니다.
프로젝트 전체가 아닙니다.

⚠️ **그것만으로는 서명이 안 될 수 있습니다.** VM 의 기본 서비스 계정에는 개인키가
없어서 `blob.generate_signed_url()` 이 IAM `signBlob` 을 거쳐야 서명합니다. 그 권한이
없으면 **`/app/gait/analyze` 가 500 으로 죽습니다** — 버킷도 권한도 다 맞는데 서명만
안 되는 상태라, 증상만 보면 원인이 안 보입니다. 두 가지 길이 있고 앞의 것이 낫습니다.

1. 그 서비스 계정에 **자기 자신에 대해** `roles/iam.serviceAccountTokenCreator`
   (키 파일이 생기지 않습니다)
2. SA 키 JSON 을 마운트 — `docker-compose.yml` 에 그 줄이 이미 주석으로 준비돼 있습니다
   (`GAIT_GCP_CREDENTIALS_FILE`)

#### B. 보관은 "탈퇴 시 지체 없이 파기" 하나다

**별도 보관 기간을 두지 않고, 모델 학습에도 쓰지 않습니다.**

근거는 우리가 이미 공개한 문서입니다. <https://sajoyo.github.io/daengs-legal/privacy.html>
(시행 2026-09-01) 의 4항이 *"계정과 이용 기록은 회원 탈퇴 시 지체 없이 파기합니다"* 이고,
1항의 수집 항목표에 **사진(피부 상태 확인)** 과 **동영상(걸음걸이 분석)** 이 이미
들어가 있습니다. 여기서 더 짧은 기간을 새로 정하면 공개 문구와 어긋난 채 굳고,
학습 이용을 열면 목적이 달라져 **선택 동의 UI·저장·철회**를 따로 만들어야 합니다.
지금 필요한 것은 그게 아닙니다.

#### 이 결정이 만드는 숙제 — 파기가 아직 반쪽입니다

**탈퇴가 DB 행만 지우고 저장소 객체는 안 지웁니다.** 그러면 위 4항을 못 지킵니다.
탈퇴 경로에서 그 사용자의 `gait/` · `territory/` · `pets/` 접두사를 같이 지워야 하고,
이건 별도 카드로 엽니다. `gait.cleanup` 태스크에 통로만 열려 있습니다(D-043).

#### 안 정한 것

- **Signed URL 만료의 최종값** — 업로드 900초 · 내려받기 600초는 잠정입니다.
  실기기 왕복을 보고 정합니다
- **설정 이름.** `GAIT_*` 가 이미 공용으로 쓰이고 있습니다(점령지 사진도 같은 변수를
  읽습니다). `MEDIA_*` 로 옮기는 것은 **버킷이 실제로 도는 것을 확인한 뒤** 별도
  카드로 합니다 — 켜는 일과 이름 바꾸는 일을 한 PR 에 섞으면 안 도는 날 원인이 둘이 됩니다
- **개발용 버킷.** 개발 서버는 당분간 `local` bridge 로 둡니다. prod 하나로 검증이
  끝난 뒤에 필요하면 만듭니다
- **산책 GPS 기록은 저장소로 안 갑니다.** 좌표는 JSONB chunk 로 DB 에 들어가고 용량은
  분할 업로드로 이미 풀려 있습니다(D-044). 파일로 뺄 이유가 없습니다
