...
#: `summary_for_ui` 의 P90−P10 범위를 낼 최소 관측 프레임 수 (walk_demo v4 값 그대로).
#: 이보다 적으면 백분위가 극값 한두 개에 좌우돼 값이 의미를 잃습니다.
#: **legacy 경로에서는 p90p10 을 계산하지 않습니다** — `build_features(p90p10=True)` 일 때만 씁니다.
P90P10_MIN_FRAMES = 5
