...
| 강수 | 초단기실황/예보 | `RN1`, `PTY`(0 없음/1 비/2 진눈깨비/3 눈/4 소나기 **/5 빗방울/6 빗방울눈날림/7 눈날림**) |
...
- 결과: **12/17 호출 성공.** 원본 응답은 로컬에만 있다 (실시간은 저장하지 않는다 — RAG-012).
  여기 남기는 것은 **모양과 계약**이다
...
| 범주 코드 | `PTY`(초단기 0~7 · 단기 0~4) `SKY`(1·3·4) `LGT`(숫자 — 0.2~100kA/㎢) | `'0'` `'1'` `'3'` `'4'` |
...
### 6.9 3차 실측 (2026-08-25 오후) — 활용신청 상태가 §6.8 에서 셋 바뀌었다

`stn_inf.php` 활용신청 승인 직후 apihub 5개 오퍼레이션을 다시 찔렀다.

| 상태 | 오퍼레이션 | §6.8 대비 |
|---|---|---|
| ✅ 200 | `typ01/url/stn_inf.php` | **403 → 200 (승인됨).** 🔴 블로커 해제 |
| ✅ 200 | `typ01/cgi-bin/url/nph-aws2_min` | 그대로 |
| ✅ 200 | `getSenTaIdxV3` | 그대로 (`03 NO_DATA` = 권한은 통과) |
| 🔴 403 | `getAirDiffusionIdxV3` | **§6.8 이 안 찔러 본 것.** 대기 중이던 "생활기상지수 활용신청"의 실체가 이것이었다 |
| ✅ 200 | `getUVIdxV3` | **403 → 200.** §6.8 이 "따로 신청해야 한다"고 적은 것이 더 이상 사실이 아니다 |

**정리 — 생활기상지수는 신청할 것이 없다.** 남은 403 은 `getAirDiffusionIdxV3` 하나뿐인데
`kma_life_index` 가 그것을 부르지 않는다. ③-b 가 UV 를 축에서 뺀 결정은 그대로 두되,
그 근거 중 "미신청이라 못 쓴다"는 이제 틀린 문장이다.

**`stn_inf.php` 가 준 것** — 745 지점, `LON`/`LAT` + `LAW_ID`(법정동코드) + `FCT_ID`.

```
401  127.02601  37.48462  …  서초  …  11B10101  1165010800  …  서울특별시 서초구 서초동
```

- 역삼(37.4979, 127.0276) 격자 **(61,125) 안에 AWS 지점이 정확히 하나** 있다 (401 서초).
  ⑤-d 1순위와 ④-e 1번이 **실제로 발동한다** — 그 격자는 초단기실황을 생략해 56→32회/일
- `LAW_ID` 는 생활기상지수 `areaNo` 재료다. 반면 `FCT_ID` 는 **동네예보구역**이라 특보구역과
  다르다 — 🟡 특보구역 매핑표는 여전히 필요하다
- `LAW_ADDR` 의 시도 정식명(`서울특별시`)이 에어코리아 `addr`·`informGrade` 의 단축명(`서울`)과
  다르다. 정규화는 `kma_apihub.sido_of` 한 곳에만 둔다 (안 하면 미세먼지 예보가 한 번도 안 걸린다)

**응답 지연 (같은 시각 2회씩)** — 8초 예산(⑤-b)을 어디에 쓰는지가 여기서 갈린다.

| | 지연 | 크기 |
|---|---|---|
| `nph-aws2_min` `stn=0` | 0.50s | 94KB |
| `stn_inf.php` | 0.25s | 146KB |
| `getUltraSrtNcst` | 0.08s | 0.1KB |
| **에어코리아 `getMsrstnList`** | **5s 타임아웃 → 재시도 3회 = 17s** | — |

에어코리아 측정소 목록만 튄다 (3회 중 1회꼴). **월 1회짜리라 요청 예산 밖에 두는 근거가
됐다** (`config.static_budget_sec`) — 콜드 캐시에서 이것 하나가 8초를 다 먹으면 판정을 못 낸다.

**그 `= 17s` 가 "예산을 늘리면 된다"를 기각한 근거이기도 하다** (2026-09-04). 재시도까지 세면
provider 하나가 남은 예산을 혼자 다 먹을 수 있어서, 직렬인 한 8초를 15초로 올려도 순서상 뒤쪽은
그대로 죽는다. 그래서 예산이 아니라 호출 방식을 고쳤다 — 요청 예산 안의 조회는 이제 병렬이고,
8초는 **총합이 아니라 가장 느린 하나**다 (⑤-b). 정적 메타가 여전히 예산 밖인 것은 안 바뀐다.

**생활기상지수 — `areaNo` 는 시군구 단위였다 (같은 날 4차 프로브).** §6.8 이 못 본 것이다.

| `areaNo` | `getUVIdxV3` |
|---|---|
| `1165000000` (서초구) · `1168000000` (강남구) · `1100000000` (서울) | **`00 NORMAL_SERVICE` · 시각별 20건** |
| `1165010800` (서초동 = 법정동) | `99 검색결과가 없습니다. [1165010800]` |
| `11B10101` (동네예보구역) | `99 검색결과가 없습니다.` |

**이 실패가 고약한 이유는 권한 문제처럼 안 보인다는 것이다** — 403 도 빈 목록도 아니고
"검색결과 없음" 이라 지역에 자료가 없는 것으로 읽힌다. 변환은 `kma_life_index.area_code()`
한 곳에 두고 테스트로 고정했다.

> **RT-002 ②-c 정정** — 거기서 "`LAW_ID`(법정동코드)가 생활기상지수 `areaNo` 재료"라고 적었는데,
> **그대로는 안 되고 앞 5자리만 쓴다.** 재료인 것은 맞고 한 번 자르는 단계가 빠져 있었다.

`getSenTaIdxV3` 는 **여전히 `03 NO_DATA`** 다. 시군구 `areaNo` 로 바꿔도, `requestCode` 를
`A01`~`A08`·`A41`·`A42` 로 훑어도, 발표시각을 셋 바꿔도 전부 같다. `requestCode` 를 빼면
`11 NO_MANDATORY` 이므로 **파라미터 이름은 맞고**, 남은 것은 자료가 안 실려 있다는 쪽이다.
...
|---|---|---|---|---|---|
| 역삼 | 61,125 | 강남대로 1.82km | 401 | UNSAFE/heat | 20:00 ~ 익일 11:00 |
| 해운대 | 99,75 | 우동 1.66km | **없음** | UNSAFE/heat | 20:00 ~ 23:00 |
| 제주 | 53,38 | 이도동 0.12km | 184 | CAUTION/heat | 19:00 ~ 익일 09:00 |

**`ncst`(초단기실황)가 해운대에서만 불렸다.** 나머지 둘은 같은 격자에 AWS 지점이 있어
④-e 1번이 발동해 생략됐다 — 격자당 56→32회/일이 설계가 아니라 **실제 동작**임이 여기서 보인다.
