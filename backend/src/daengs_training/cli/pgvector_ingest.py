        ...
        for start in range(0, len(data), args.batch_size):
            ...
            with conn.cursor() as cur:
              for r, vec in zip(batch, vectors):
                ...
                cur.execute("""INSERT INTO training_rag_chunks(chunk_id,document_id,chunk_index,text,token_count,metadata,embedding_model,embedding,content_sha256) VALUES(%s,%s,%s,%s,%s,%s,%s,%s::vector,%s) ON CONFLICT(chunk_id) DO UPDATE SET text=EXCLUDED.text,token_count=EXCLUDED.token_count,metadata=EXCLUDED.metadata,embedding_model=EXCLUDED.embedding_model,embedding=EXCLUDED.embedding,content_sha256=EXCLUDED.content_sha256""",(r["chunk_id"],doc_id,r["chunk_index"],r["text"],token_count,json.dumps({"kinds":r.get("kinds",[]),"heading_path":r.get("heading_path",[])}),label,"["+",".join(str(float(x)) for x in vec)+"]",content_sha))
            conn.commit()
