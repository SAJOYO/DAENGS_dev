...
def summer_feels(ta: float, rh: float) -> float:
    """기상청 여름철 체감온도 (**2022-06-02 개정판**). **풍속을 쓰지 않는다.**

    그래서 여름에는 바람이 판정에 반영되지 않는다 — 체감상 시원해도 공식이 반영하지 않는데,
    공식을 그대로 쓰는 원칙(③-c)을 따른다.

    ✅ **RT-004 에서 검산했다** — 공표형과 계수까지 항등이다. 그 대조는
    `test_rules.py::test_the_summer_formula_matches_the_published_one` 이 들고 있다
    (공표형을 다른 항 묶음으로 다시 적어 비교하므로 옮겨 적기 오타가 거기서 잡힌다).
    ⚠️ 이전 주석의 `(2020~)` 은 틀렸다 — 2020 년 것은 '더위체감지수'이고, 습구온도를 쓰는
    지금 식은 2022-06-02 부터다.
    """
...
def winter_feels(ta: float, wind_ms: float) -> float:
    ...
    ✅ **RT-004 에서 검산했다.** 공표식의 `V` 가 **km/h** 라 아래 3.6 이 필수인데, 빠뜨려도
    예외가 안 나고 값만 조용히 따뜻해진다. 그 단위까지 테스트가 잡는다.
    """
