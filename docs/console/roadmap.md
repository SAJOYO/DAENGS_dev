...
- **첫 카드는 신고가 아니라 그 앞의 것들이었다.** 감사 로그(#203) · 역할별 계정 발급(#207) · 회원 조회(#211 ·
  ...
  **신고(A1)를 막는 것은 이제 운영 DB 적용뿐이다** (§5).
...
| 사람들이 무엇을 묻나 | ✅ **이미 콘솔** (#223) — 능력별 분포 · 응답 결과 8값 · 처리 실패. 다만 **아직 볼 값이 없다** (개발 DB `chat_turns` 0건) — 숫자는 운영 DB 에 붙어야 처음 맞아 본다 | 요청 메타데이터(지연 · request_id)까지 | B2 |
| 크롤이 실패했다 | ✅ 이미 콘솔 (#76) | — | — |
...
## 2. 지금 상태 (2026-09-04, `origin/dev` `d749f39` 실측 — #222 · #223 까지)
...
| A1 | **AI 답변 신고** — `answer_reports`(turn_id FK · app_user_id · reason · status open/reviewed/dismissed · reviewed_by) · `POST /app/reports` · `GET /admin/reports` · `/console/reports` 목록 + 상세(turn 원문 · `public_response`) + 처리 | §3 · §1 첫 줄 | #131 ✅ 머지됨 · **운영 DB 적용**(`main` 에 아직 없다) · A0 ✅ D-053 · A4 ✅ · 앱 카드(DAENGS_APP)는 **전제가 아니다**(D-053 ④) | M | ⬜ 운영 DB 적용 뒤 |
...
B3 대화 집계 ✅ → B4 운영 지표 화면 ✅   (둘 다 #223. 화면은 섰고, 숫자는 운영 DB 에서 처음 맞아 본다)
...
- **A1 이 맨 뒤인 이유** — 전제가 셋(#131 · A0 · A4)이었다. **셋 다 닫혔고 남은 것은 운영 DB 적용뿐**이다.
