...
`score` 를 돌리기 전에 이것이 통과해야 한다. 사람 라벨 30개가 모이기 전까지 judge 를 믿을
근거는 이 앵커뿐이고, 앵커가 깨지면 그 판정 파일은 버린다.

────────────────────────────────────────────────────────────────────────────
앵커는 **의견이 아니라 확인 가능한 사실**로만 짓는다
────────────────────────────────────────────────────────────────────────────
`RAG-075` 가 뽑은 뿌리가 이 규칙의 이유다. 세 번 반복된 실패가 하나였다:
...
셋 다 *"재는 자 없이 판단했다"* 다. **"이 답변은 좋다/나쁘다" 로 앵커를 지으면 그 실패를
앵커에 그대로 넣는 것이다.** 그래서 여기 있는 것은 전부 이 모양이다:
