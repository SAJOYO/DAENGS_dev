...
class PetDisplayUpdate(BaseModel):
    """**이름만** 바꿉니다 (`PATCH /app/pets/{pet_id}/display`, MVP 결정 §5).

    이름·사진은 보호자마다 자기 값입니다 — A 의 `롱이씨`와 B 의 `롱롱씨`를 연결해도 각자
    자기 이름을 봅니다. 그런데 기존 `PUT /app/pets/{pet_id}` 는 **전체 PUT** 이라, 연결된
    B 가 이름만 고치려 해도 응답으로 읽은 **A 의 공통 정보**(견종·생일·지병·배웅)가 B 의
    원본 행에 그대로 덮어써집니다. 그래서 전체 PUT 은 연결된 비(非)그룹주보호자에게
    409 로 막고, 이름은 이 좁은 계약으로 엽니다.

    사진은 이미 티켓·confirm 이 따로라(`/app/pets/{id}/photo*`) 여기 없습니다 —
    그 경로들은 계속 **행 대표**만 통과합니다.
    """
...
class PetResponse(BaseModel):
    ...
    #: **내가 이 논리 강아지 그룹의 주보호자인가** (MVP 결정 §6).
    #:
    #: `is_owner` 와 갈라 둔 것이 의도입니다. 같은 실제 강아지를 A 와 B 가 각자 등록해
    #: 연결하면 B 는 **자기 `pets` 행의 대표**라 `is_owner` 가 true 지만, 그룹의 주보호자는
    #: 초대한 A 입니다. 그래서 B 에게 열리는 것은 **자기 이름·사진 수정**뿐이고,
    #: 초대·내보내기·승계·삭제·배웅·공통 프로필 수정은 A 만 할 수 있습니다.
    #:
    #: 연결 안 된 강아지는 `is_owner` 와 **언제나 같은 값**입니다 — 구 앱이 이 필드를
    #: 몰라도 지금까지와 똑같이 동작하는 이유입니다.
    #:
    #: ⚠️ 앱이 이 값으로 버튼을 가리지 않아도 **서버가 다시 봅니다**(409). 구 앱이 그대로
    #: 호출할 수 있으므로 화면에서 숨기는 것만으로는 부족합니다.
    is_group_owner: bool = True

    #: **이 논리 강아지 그룹에 나 말고 다른 보호자가 한 명이라도 있는가** (docs/co-care.md
    #: §6 「목록 필드 — `has_other_carers`」).
    #:
    #: 카드 한 장이 가리키는 그룹 전체를 봅니다 — 연결 안 된 아이는 그 행의 대표 ∪ 돌보미,
    #: 연결된 아이는 그룹 모든 행의 대표 ∪ 각 행의 돌보미. **사용자 id 로 중복 제거**하고
    #: **부른 사람을 뺀 뒤** 남는 사람이 있으면 true 입니다. 앱은 이것으로 그룹 주보호자
    #: 본인 카드에도 「공동 돌봄」 뱃지를 붙입니다(`is_owner`·`is_group_owner` 로는 못 가립니다).
    #:
    #: 이름은 연결 차단 사유 `has_other_carers`(`services/pet_member.py`)와 같지만 뜻이
    #: 넓습니다 — 저건 "받는 사람의 행에 돌보미가 있다", 이건 "그룹에 나 말고 보호자가 있다".
    #:
    #: 기본값 false 는 구 응답(필드 없음)을 읽는 앱이 뱃지를 안 그리게 하려는 것입니다.
