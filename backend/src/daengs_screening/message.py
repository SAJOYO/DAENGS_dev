...
#: ★ **계열 한 줄을 화면에 띄울 것인가** — **켜짐 (2026-09-08 사용자 결정)**.
#:
#: STEP 28~33 이 실측했습니다. 6종 이름은 holdout 커버리지 41.1% 로 못 쓰는데,
#: **계열 4군은 67.9%** 입니다 (긴급도 하향 3.7% · A6 오명명 12.5%, 둘 다 관문 안).
#: 하향 방지 규칙(STEP 35)까지 걸면 실제로는 **66.5%** 입니다.
#: 즉 *"이름은 못 말해도 계열은 말할 수 있다"* 가 실측으로 확인됐습니다.
#:
#: **켤지는 모델 판정이 아니라 제품 결정이었고, 2026-09-08 에 켜기로 정했습니다.**
#: 끄려면 환경변수 `DOG_SKIN_SHOW_GROUP=0`.
#: 판단 자료: `docs/결정_계열_이름을_켤_것인가.md`
#:
#: ⚠️ **켠 상태에서 반드시 지켜야 할 것 둘**
...
def lesion_group_line(dist: list[tuple[str, float]],
    ...
    **더해서** 구합니다 (argmax 를 묶는 게 아니라).
    ...
    #   당했습니다). 여기는 **문장만** 만듭니다.
    from daengs_screening.agent import lesion_group
...
def compose_screening_message(pred: Prediction, abnormal_p: float | None = None) -> str:
    ...
    **여섯 개 전부** 보여주고, 어느 쪽인지는 판단할 수 없다고 말한 뒤 진료를 권합니다.
    ...
    from daengs_screening.agent import a6_alert, lesion_group_dist          # noqa: E402
