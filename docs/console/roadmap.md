...
| 감사 로그 | ✅ `admin_audit_log` (#203). 남는 것은 로그인 시도 넷과 계정 발급·role 변경·정지·해제(#207), 본인 비밀번호 변경(#222). **보는 화면은 아직 없다** (A4-1) — 그 선(감사 로그 = DB 테이블 / 운영 로그 = 파일)은 §6 | #203 · #207 |
...
| A4-1 | **감사 로그 조회 화면** `/console/audit` — 로그인 시도 · 계정 변경 · **개인정보 원문 조회** | #203 이 테이블만 만들었고 보는 길이 psql 뿐이다. #207 · #212 · #222 가 기록을 아홉 종류로 늘렸고, 그중 `pii_revealed` 는 **이 테이블이 생긴 첫째 이유**다 (§1). **`AUDIT_ACTIONS` 가 유일한 목록이니 화면은 거기서 세라** — 여기 적힌 수는 카드가 하나 머지될 때마다 낡는다 | #203 · #207 · #212 · #222 | S | ⬜ |
...
A4 감사 로그 ✅ → A3 계정 관리 ✅ → A2a 회원 조회 ✅ → A2b 복호화·상태 변경 ✅
                                                        (A 트랙. A4 가 앞인 이유는 아래)
남은 A 트랙: A0 결정(사람) → A1 신고 · A4-1 감사 로그 화면 · A5 보존
