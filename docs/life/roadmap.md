...
| `/life/walk-conditions` | 결정적 · T+24h 타임라인 · `sources` 노출 · 판정 불가는 `unknown`(503) | RT-001 ⑥ |
...
| REFUSED | **없음** — 산문 물러섬이 OK 로 통과 (D-035 가 수용한 v1 한계) | `medical` · `emergency` 를 REFUSED 로 (A3a). `place` 는 라우터 몫(A3b), `training` 은 다중 EXECUTE | — | — |
...
| A5 | **특보구역명 ↔ 행정구역 매핑표** (`data/reference/`) | `collect.py` 가 `warning_area=None` — **구 단위 특보를 놓친다.** `/life/walk-conditions` 의 유일한 기능 구멍 (RT-001 ②-a · RT-002 ②-c) | 없음 | S | ⬜ |
...
C5 만 남았다 (서버가 있는 날).  D 는 틈에.  F0 정찰은 A3a 와 병렬.  F2 분실 툴은 독립.  F3 장례 문항은 언제든.
```
