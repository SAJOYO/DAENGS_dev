...
카드 **#496**(첫 카드) → #537(앱 경로) → #543(한도) → #544(GPU 서비스·FLUX.2-klein-4B) → #557(FLUX.2-klein-4B 실험) → #572(12달 열기 + 2장 뽑기) → #592(콘솔 경로 + 딸기·상추 틀) → **#593(앱에도 딸기·상추 — D-085)**. 이 폴더는 그 흐름의 **세션 인수인계 문서**입니다.
...
| 한도 규칙 | 동시 1장 · 강아지마다 **카드 종류당 한 장**(보호자마다 따로, 지우면 그 카드는 다시 열림 — #593 D-085 로 「달마다」에서 넓어졌다. 세는 칸이 `month` 가 아니라 `card_key` 라 4월 카드가 딸기를 막지 않는다. 하루 한도는 달·종류가 계수기 하나를 나눠 쓴다) · 제목에만 쓰는 `title_name` · 목록의 남은 횟수 — **D-077, #572 로 D-084 가, #593 으로 D-085 가 개정.** 세는 단위가 카드 장수에서 **요청(`pick_group`) 하나**로 바뀌었다 — 요청은 Nano Banana 2 경로에서 카드 1장(+ 닮음 미달이면 재시도 한 번), GPU 경로에서 최대 `cardimage_pick_count`(2)장을 순차로 만들고 하나를 고른다(위 「한 요청의 장수」). 닮음 미달 요청은 하루 한도를 안 쓰되 지울 수 없는 시도 표시로 유료 실패 상한(5)에는 걸린다 | 사용자 결정 09-15 (#543) · D-084(#572, 09-16). 앱 짝은 `DAENGS_APP#414` |
| 카드 키 | 카드를 가리키는 값이 달 정수에서 `catalog.CardSelector = int \| str` 로 넓어졌다. 1~12 는 달, `catalog.KINDS` 의 문자열은 종류다. 저장·로그·API 는 `catalog.card_key()` 가 만드는 문자열(`"4"` · `"strawberry"`)을 쓴다. **앱 표 `ai_cards` 에도 칸 `card_key`(NOT NULL)가 생겼고 `month` 는 nullable 이다** — 종류 카드는 달이 없다 | #592 (09-18) 가 카탈로그까지, **#593 (09-18, D-085) 이 앱 표와 앱 경로까지.** 앱 경로는 이제 달 정수만 넘기지 않는다 — 아래 「앱 계약」 |
| 앱 계약 (전환기) | `POST /app/ai-cards` 가 `month`(1~12)와 `card`(달 숫자 또는 종류 문자열)를 **둘 다** 받는다. `month` 만 보내는 옛 앱은 지금과 똑같이 돈다. 둘 다 보내면 같은 카드일 때만 통과하고 어긋나면 400 `card_conflict`, 둘 다 없으면 400 `card_required`. 응답은 `month`(종류 카드는 `null`)와 `card` 를 함께 싣는다. 404·409 코드는 **고른 카드**를 따른다 — 달은 `month_closed`·`month_taken`, 종류는 `card_closed`·`card_taken`. 규칙은 `routers/ai_card.py::_selector` 한 곳 | #593 (09-18, D-085 — 사용자 결정 A). **`month` 를 언제 걷어낼지는 앱 배포에 달렸다**(날짜가 아니라 조건) |
| 과일·채소 카드 | 딸기 **BERRY**(배지 `NEO-S0824`, 부제 `FRUIT DOG`) · 상추 **LETTUCE**(배지 `NEO-0824`, 부제 `VEGGIE DOG`) 둘을 `catalog` 에 넣었다. 틀은 `cardimage/strawberry_template.webp` · `lettuce_template.webp`(994×1582, `backend/tools/cardimage_fruit_templates.py` 로 제목만 지우고 5:8 로 맞춘 것). 본문에 강아지 **얼굴만** 보이는 틀이라 프롬프트 갈래가 하나 더 생겼다 — `engine.build_prompt(face_only=True)`(10월 `face_hidden` 의 반대, 셋은 배타). **#593 부터 앱 경로(`card=strawberry`·`card=lettuce`)에서도 뽑힌다** — 종류 카드에는 `DAENGS_CARDIMAGE_MONTHS` 같은 잠금이 없고 **카탈로그(`catalog._KIND_CARDS`)에 있으면 열린 것**이다 | #592 (09-18) 콘솔 · #593 (09-18, D-085) 앱. 틀 채택 `0284c41f`. 실호출 스모크 3장은 잠정 「됨」(상추 아래 띠에 없던 아이콘 둘 — `worklog.md` 09-18 절). 남은 21종·seed 목록은 안 정했다(`roadmap.md` 4번) |
| 콘솔 저장 표 | 콘솔이 뽑은 카드는 앱 표(`ai_cards`)가 아니라 **`admin_ai_cards`** 에 남는다 — 한도·세마포어·큐를 쓰지 않는 별도 경로다. 저장 키는 `admin-ai-cards/{admin_user_id}/{card_id}.png`(`core/storage.build_admin_ai_card_key`)로 앱 카드(`ai-cards/`)와 접두사가 다르다. **사진 원본은 저장하지 않고 만들어진 카드만** 남긴다. 저장소가 꺼져 있으면 행을 안 만들고 응답의 `stored` 가 거짓이다 | #592 (09-18). 스키마 원본 `db/init/41_admin_ai_cards.sql`, 이미 도는 DB 용 `db/migrations/2026-09-18_admin_ai_cards.sql` |
| 콘솔 저장 목록 | `GET /admin/cardimage/cards` 가 **기본 10줄 + `next_cursor`**(키셋, `(created_at, id)` 튜플 비교)를 준다 — 그 값을 `cursor` 에 그대로 넣으면 다음 쪽이고 `null` 이면 끝이다. 손으로 고친 커서는 422 `bad_cursor`. 화면에 「더 보기」와 줄마다 「PNG 저장」(`<카드>_<강아지>_<시각>.png`)이 붙었다. 일괄 삭제·자동 정리는 **안 한다** | #593 (09-18). 사용자 결정 09-18. 옛 동작은 최근 50장에서 끊겼다 |
| 콘솔 엔진 선택 | `POST /admin/cardimage/generate?card=&engine=&seed=` — `engine` 은 `gemini`(Nano Banana 2) · `cardgen`(FLUX.2-klein-4B) 둘. `cardgen` 은 `DAENGS_CARDGEN_URL` 이 있어야 하고 없으면 503 `cardgen_disabled`, `gemini` 에 `seed` 를 주면 400 `seed_not_supported`(Nano Banana 2 는 seed 를 버린다). 없는 카드는 404 `card_closed`. 화면이 그릴 목록은 `GET /admin/cardimage/options`(카드 14 · 엔진 2 · `catalog.PHOTO_GUIDANCE`)에서 받는다 — 프론트에 복제하지 않는다 | #592 (09-18). **운영 앱 경로의 엔진 선택은 그대로다** — 여전히 `DAENGS_CARDGEN_URL` 이 비어 Nano Banana 2 한 장 |
| 제목이 판을 넘칠 때 | 대문자 높이를 `CAP_HEIGHT`(48)에서 `CONDENSE_CAP`(26)까지 줄여 보고 → 그래도 넘치면 장평(가로 축소)을 1.0 에서 `CONDENSE_MIN`(0.70)까지 → 그래도 넘치면 높이를 `MIN_CAP`(20)까지 → **그래도 넘치면 그대로 그린다.** 판정은 글자 잉크 **맨 아래 높이**에서 사선 경계를 잰다. 1·3·5월 앞말은 판에 안 들어가 `SEBAE` · `SCHOOL` · `HOME` 으로 줄였다(원본 `NEW YEAR` · `FIRST DAY` · `HOME TEAM`) | 사용자 결정 09-18 (#592). 눈으로 본 격자 `cardimage/out/_title_check/titles_momo.png` · `titles_korean.png` · `titles_princess.png`(이름 하나에 14장 — `MOMO` · `보리` · `PRINCESS BUTTERCUP`) |
