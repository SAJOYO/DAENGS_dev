...
- 이 서비스에는 인증이 없습니다 (스크리닝과 같은 상태 — D-024 · D-029). compose
  `profiles: ["gait"]` 뒤에 꺼둔 채로 들어온 이유가 그것이고, **켜는 시점은 사람이
  정합니다.**

소유권을 서버에서 강제하려면 계정·반려견 프로필과 엮어야 하고, 그것은 기록 저장 구조가
정해진 뒤입니다 (`docs/gait-record-data-design.md` TBD 1).
