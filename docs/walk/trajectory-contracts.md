...
이 코드는 `daengs_walk`의 순수 내부 계약이다. 현재 서비스에서 호출하지 않는다.
새 GPS 판정기, 관측 경로/보행선 투영, 실시간 그림자 계산, HTTP·DB·Room·APP 화면은
후속 단계다. 기존 motion-v1과 WalkFacts 계산을 변경하지 않았다.
...
순번은 위치만이 아니라 시작·일시정지·재개·유실·종료 제어 사건에도 부여된
원본 저널 순번이어야 한다. 기존 `client_seq`와 대응시킬 때 명시적 변환이 필요하다.
...
2. 기존 영속 저널/precision 백업을 이 참조 계약에 연결한다. 위치·속도 근거를 유지하고,
   기존 motion-v1의 보행 단절을 관측 경로 단절로 자동 변환하지 않는다.
3. 새 관측 연결 판정과 보행 투영을 그림자 계산으로 실행한다. 같은 최종 저널에서
   일괄/누적 처리, 전송 경계 변경, Kotlin/Python 결과를 대조한다.
