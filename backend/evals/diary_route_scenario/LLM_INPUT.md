...
2026-09-14, #520. 현재 기본 카드 작성은 `diary-prose-input-v5`를 사용한다.
...
공원은 이름·종류·거리 숫자 없이 가까운 공원의 존재만 전달한다. 내부 원자료와 인용은 보존한다.
실제 v5 서술과 남은 오류는 [이동 재료 Gemini 검토](movement-gemini-01/REVIEW.md)에 남겼다.
...
- 활동: 카드 범위에 맞춰 모양·되짚기·속도의 의미를 조립한 materials를 보낸다.
  지속 흐름은 한 번 적고 변화가 적용되는 부분을 연결한다. 전환 사건은 순서로 표현한다.
  선택적 행동핀은 확인된 이름·기록된 행동과 이동 부분의 연결만 받는다.
  원 GPS·시각·지속시간·속도 숫자·임계값·지지 배열은 보내지 않는다.
...
아래 수치를 현재 v4의 토큰 수·품질·운영 지연으로 사용하지 않는다.
