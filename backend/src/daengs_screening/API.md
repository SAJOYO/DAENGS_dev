...
```jsonc
{
  ...
  "stage2": {
    ...
    //    아무것도 안 숨깁니다.
    "groups": [
      {"name": "미란·궤양", "prob": 0.8843, "percent": 88.4},
      {"name": "융기·발진", "prob": 0.1092, "percent": 10.9},
      {"name": "결절·종괴", "prob": 0.0065, "percent": 0.7},
      {"name": "표면 변화", "prob": 0.0,    "percent": 0.0}
    ],

    // ★ 계열 **주장** 한 줄. 확신(p(이상) × p(계열)) < 0.469 면 **null** —
    //   그때는 통째로 안 그립니다 (약 3분의 1). `text` 를 그대로 띄우세요.
    "group": {"name": "미란·궤양", "prob": 0.8843, "percent": 88.4,
              "confidence": 0.8101,
              "text": "모양만 보면 미란·궤양 계열에 가깝습니다.",
              "caveat": "진단이 아닙니다. 같은 계열 안에서도 원인 질환은 여럿입니다."},

    // ★ "덩어리가 의심됩니다" 경보. `p(이상) × p(A6)` < 0.40 이면 **null**.
    //   유일하게 병변 이름을 말하는 자리입니다 (아래 "표시 규칙" 참고).
    "alert": null,

    // 병변 6종. ⚠️ **앱은 안 그립니다** — 관리자 콘솔이 씁니다.
    //   계약에 남겨두는 이유: 화면 결정이 뒤집혀도 앱 한 줄이지 규격 변경이 아닙니다.
...
### 앱 쪽 화면 규칙 — ★ 2026-09-08 개정

카드는 **위에서 아래로** 이렇게 그립니다:

```
1. alert  이 null 이 아니면  →  맨 위 경고 상자 (text + action + caveat)
...
2. **`distribution`(6종)은 안 그립니다.** 계약엔 있지만 보호자 화면엔 안 띄웁니다
   ⚠️ 계열 4묶음은 6종을 **자른 게 아니라 더한 것**이라 숨기는 게 없습니다
...
6. **면책은 두 번까지.** 서버가 이미 4·7 두 번 합니다. 앱이 더 붙이지 마세요 —
   다섯 번 하던 걸 줄인 것입니다. 반복하면 아무도 안 읽습니다
7. **`group` / `alert` 가 `null` 이면 그 줄을 통째로 안 그립니다.** 확신이 낮을 때
   (약 3분의 1) `group` 이 null 이 되고 막대만 남습니다 — 그게
   *"확신 없으면 말하지 않는다"* 의 구현입니다

#### ⚠️ `alert` 만 병변 이름을 말합니다

`p(이상) × p(A6) ≥ 0.40` 이면 **"덩어리가 의심됩니다"** 를 맨 위에 씁니다.
계약 전체가 *"이름을 말하지 마라"* 인데 여기만 예외인 이유:

* 임상 해설이 *"A6 으로 오탐하는 건 상대적으로 안전"* 이라 적어뒀습니다
  (병원에 가서 확인하면 되니까) — **반대 방향(놓침)이 훨씬 나쁩니다**
* A6 은 **종양 감별**이 필요한 유일한 클래스입니다
* 문턱은 **재현율로** 정했습니다 (0.40 → val 61.1% / holdout 59.4%).
  ⚠️ **정밀도로 바꾸지 마세요** — 정밀도는 모델이 아니라 **모델 × 유병률**의
  성질이라 문턱을 고정해도 안 고정됩니다

되돌리기는 backend 환경변수 두 개, 각각 독립입니다:
`DOG_SKIN_SHOW_GROUP=0`(계열 한 줄) · `DOG_SKIN_SHOW_A6_ALERT=0`(덩어리 경보).
